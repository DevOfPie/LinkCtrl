...
const answerUpdateCheck = `-- name: AnswerUpdateCheck :execrows
UPDATE instance_settings
   SET update_check_enabled = $1::boolean,
       updated_at = now()
 WHERE id
...
func (q *Queries) AnswerUpdateCheck(ctx context.Context, enabled bool) (int64, error) {
	result, err := q.db.Exec(ctx, answerUpdateCheck, enabled)
	if err != nil {
		return 0, err
	}
...
const claimUpdateCheck = `-- name: ClaimUpdateCheck :execrows
UPDATE instance_settings
   SET update_checked_at = $1::timestamptz,
       updated_at = now()
 WHERE id
   AND update_check_enabled IS TRUE
   AND (update_checked_at IS NULL OR update_checked_at <= $2::timestamptz)
`
...
func (q *Queries) ClaimUpdateCheck(ctx context.Context, arg ClaimUpdateCheckParams) (int64, error) {
	result, err := q.db.Exec(ctx, claimUpdateCheck, arg.At, arg.NotSince)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const setUpdateCheckEnabled = `-- name: SetUpdateCheckEnabled :execrows
...
func (q *Queries) SetUpdateCheckEnabled(ctx context.Context, enabled bool) (int64, error) {
	result, err := q.db.Exec(ctx, setUpdateCheckEnabled, enabled)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const updateCheckAnswered = `-- name: UpdateCheckAnswered :one
...
func (q *Queries) UpdateCheckAnswered(ctx context.Context) (bool, error) {
	row := q.db.QueryRow(ctx, updateCheckAnswered)
	var answered bool
