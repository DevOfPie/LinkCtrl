...
WITH removed_memberships AS (
    ...
    DELETE FROM addon_identity_links il WHERE il.user_id = $1 RETURNING 1
)
...
WITH pending AS (
    ...
    UPDATE notifications n
       SET data  = jsonb_set(n.data, '{email}',
                             to_jsonb($2::text)),
           title = replace(n.title, n.data->>'email', $2::text)
      FROM batch b
