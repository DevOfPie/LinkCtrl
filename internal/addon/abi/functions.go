...
var Functions = []Function{
	...
	},
	{
		...
		Doc: "ConfigGet reads one of this add-on's own settings. The key must be one the " +
			...
			"is what this function answers on the add-on's next invocation, and one " +
			"already inside a call reads what it read.",
	},
	{
	...
	},
	{
		...
		Doc: "NetworkFetch makes one outbound request from the host and hands you what came " +
			...
			"the ErrDenied every function outside that subset returns. **One call is " +
			"one request**: a response too large for the buffer you offered is held by " +
			"the host and handed to your retry rather than fetched again, which is the " +
