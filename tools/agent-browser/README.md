...
| `specs/qr-tab-controls.spec.mjs` | kept spec (M50.8): signed in, the size slider and the number follow each other, the tab's own tooltip shows anywhere on the button and on focus where a disabled button refuses a pointer, the row controls hover visibly on the selected row, a save returns to where the reader was and a refused htmx write leaves no position behind, and with scripts off the form still saves |
