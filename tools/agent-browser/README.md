...
| `specs/workspace-control.spec.mjs` | kept spec (M46.6): signed in at 360px, the workspace switcher's closed face shows no text and the header does not overflow. **The one spec that signs in for itself**: it drives a real workspace switch, and a switch inside the shared session reaches every spec running beside it — which made the suite non-deterministic for exactly as long as it shared one |
...
| `global-setup.mjs` | signs in **once** for the whole run and saves the storage state every spec starts from (M70, F333). Twenty specs signing in as one address against `LINKCTRL_LOGIN_RATE_PER_MIN` (ten) failed the tail of every full run at `/login`, reporting it as a broken instance. It refuses to write a state holding no cookies, because a run that starts signed out fails about whatever each spec touches first and never names a session |
| `credentials.mjs` | the account, from `LINKCTRL_UI_EMAIL`/`LINKCTRL_UI_PASSWORD` or the table in `docs/dev-notes/instances.md`. One copy: six specs carried an identical private one while each signed in for itself |
