...
with one open exception stated below), the
...
**A dispute can keep one of its two labels — known, open, and stated here rather
than after it is fixed.** A `destination_disputes` row that was *filed* and
*decided* by accounts the same sweep erases keeps one of the two addresses, and
which one is not defined. The
two label columns are written by two separate statements inside the pass, and
Postgres applies one of two such statements against a single row and silently
drops the other. The ordinary case — one erased account, a dispute somebody else
decided — is unaffected, and every other column named above is scrubbed as
described.

It reaches the same audience the rest of that residue does: `destination_disputes`
is read through the instance review queue rather than through `audit.read`, and
nothing expires it. It is tracked as
[F187](build-notes/deferred-findings.md#open) and repaired by merging the two
statements into one, the way the audit pair already is.
