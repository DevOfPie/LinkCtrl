...
| Uploaded content | **This product accepts two kinds of file and no third: a QR code's logo, and an add-on's module with the manifest that describes it.** They are not alike and are not defended alike, so this row takes them in turn — the logo at length, and the add-on in the last paragraph. **About the logo, it refuses everything else.** **PNG and JPEG only, chosen by sniffing the leading bytes**: neither the filename nor the part's declared `Content-Type` is read, asserted by a source scan over the handler as well as by behaviour, so a decoder cannot be selected from outside. **An SVG is refused by name**, because an SVG is a document that carries script, external references and an entity resolver, and accepting one would mean sanitising markup this product then serves; PNG and JPEG decode to pixels, which is a smaller thing to be wrong about. **Every bound is a number, and they are enforced in the order that makes them bounds.** A decompression bomb is a small file declaring an enormous image, so the request body is capped first at **1,048,576 bytes** by `http.MaxBytesReader` — the whole multipart envelope, not just the part — and then `image.DecodeConfig` reads the *header only* and the dimension cap is enforced there: **1024 pixels a side**. Only then is anything decoded. A test feeds a file that passes the first cap and fails the second and asserts the allocation never happened, measured rather than argued. **The figures in this sentence changed at M50.5's 2026-08-12 reopening (F214), and they changed upward.** There were two header caps — the side, and 262,144 pixels in total — and an 813×813 upload passed one and failed the other, which is a refusal naming two numbers with no verdict in it and nothing the reader can do. The area figure stopped refusing anything: it is now the size a stored logo is **resized down to**, keeping its aspect ratio, with both sizes reported to whoever uploaded it. So the side cap is the only header refusal and it is what bounds the decode, at 1024 × 1024 = **1,048,576 pixels**. **A decoded pixel is eight bytes and not four**, which every earlier statement of these figures on this page got wrong in the same way: four bytes is `image.NRGBA`, which is what this product *normalizes* to, while `image/png` hands back `image.NRGBA64` or `image.RGBA64` — eight bytes a pixel — for any bit-depth-16 file, and such a file is ordinary rather than exotic. A 1024×1024 16-bit PNG of one flat colour is about ten kilobytes on the wire, so it clears the body cap by two orders of magnitude and the side cap exactly. Refusing bit depth 16 to keep the smaller figure true was considered and rejected: it would add a refusal to the change whose purpose is to stop refusing what can be adapted, and it would turn away a valid PNG for a property its author cannot see. So the decode bound is 1,048,576 × 8 = **8,388,608 bytes**, four times what the shipped area cap admitted, and it is **measured against a real decode** rather than multiplied out — a test builds that exact file, puts it through the decoder and compares the buffer that comes back. The **image buffers** one upload holds are the upload itself — live across the decode, and inside the same 1,048,576-byte body cap — plus the decoded source, the resampler's own `NRGBA` copy of it and the resampled destination: 1,048,576 + 8,388,608 + 4,194,304 + 1,048,576 = **14,680,064 bytes, 14 MiB exactly**, against 4 MiB before. **The encoder is bounded separately and by the standard library rather than by these caps**, which is why it is stated rather than folded in: the `bytes.Buffer` the PNG is written into grows by doubling, so at its last growth two arrays are live, under 3,145,728 bytes together; and flate's window, its hash tables and the per-scanline buffers are a fixed cost of one `png.Encode`, measured at about 850,000 bytes and the same whatever the image. Under 4 MiB together, so **under 18 MiB in flight per upload**. The handler's own read buffer doubles the same way and does not raise that figure — it reaches its largest size before anything is decoded, and what it hands on is the first term above. That is what accepting a large image instead of refusing it costs, every term is a constant or a measurement rather than an argument, and it is why uploads have a rate limit bucket of their own. **What is stored is a PNG this product re-encoded from the decoded pixels**, never the received bytes — which kills polyglot files, strips every kind of metadata, and means what is served later is bytes this instance produced. The re-encoding is bounded too, at **1,060,000 bytes**, and that bound is checked rather than derived, so a row can never exceed it. **Nothing about what is stored derives from user input.** Under D134 a logo is a `bytea` column on `qr_codes`, so there is no path and no object key to escape: the row is addressed by a uuid and a slug this server generated. **Decoding happens off the redirect path** — it is an authenticated `links.update` write on the dashboard tree, and no SLO re-verification is owed. The decoders are `image/png` and `image/jpeg` from the Go standard library, so no module joined the dependency set. Uploads carry a rate limit of their own (`UPLOAD_RATE_PER_MIN`, default thirty a minute, shared through Redis) **in addition to** the API limit, because an upload's cost is set by its content where a JSON body's is set by its shape. **Four addresses accept a file and all four share one bucket** — the two API `PUT`s (the default code's shorthand and the named-code collection), the dashboard's upload form on the QR tab *(the popup panel that first carried it folded into the tab at M48's 2026-08-11 reopening; the address and its bucket are unchanged)*, and since M67 the add-on install `POST` — so alternating between the API and a browser does not double the budget, which is the reasoning `login` has carried since it was first shared between the two surfaces. **The fourth address sends a larger body than the figure above bounds, and it shares the bucket anyway**, deliberately: what the bucket is about is a request whose cost is set by its content, and that is as true of a module about to be compiled as of an image about to be decoded. Its own figure is in the last paragraph, and what one address spends on installs it cannot also spend on logos. **The accepted cost, stated rather than discovered**: binary lives in the database row and therefore in every `pg_dump`, bounded at 1,060,000 bytes a code and about 20 MiB for a link at the twenty-code cap. **Since M50.6 the stored image is also drawn into the code**, and three things follow. It travels inside the SVG as a **base64 `data:` URI** — the alphabet is `A-Za-z0-9+/=`, which cannot express a quote or an angle bracket, so workspace bytes inside a drawing this product writes still cannot close an attribute or open a tag; the alternative, an endpoint serving the bytes and a reference to it, does not exist and is not wanted. The page's `img-src 'self' data:` is what permits the embedded form, and **a test now pins that directive exactly** rather than checking it contains a substring — before this the CSP tests covered `script-src` and the absence of `unsafe-` only, so widening it to `blob:` or a host would have passed everything. And **the rasteriser's allocation figure changes when a code carries a logo**: the composited image is four bytes a pixel rather than the two-colour paletted one byte, so the largest buffer a request can cause becomes 2048 × 2048 × 4 = **16,777,216 bytes** in place of 4,194,304, with the resampled logo bounded separately at 512 × 512 × 4 = 1,048,576 — the same 262,144 pixels a stored logo is fitted to, which the drawing path re-checks on the way in because that path decodes a *stored* image and a row past it could only have been written by hand. **M50.6's 2026-08-12 reopening put a third buffer beside those two**, and it is stated because the sentence above was complete only while the box was small: the occluded square grew from a fifth of the code's width to **three tenths**, so at the 2048-pixel cap the box is 614 pixels against the 512 the raster is clamped to, and the drawing path now scales that clamped raster **up** to the rectangle the box needs. That one is bounded by the box rather than by the clamp — 614 × 614 × 4 = **1,507,984 bytes**, which is `MaxSize` × the fraction, squared, times four — so the three together are under 19,400,000 and the figure that bounds a request is still the picture's 16,777,216. None of the three is on the redirect path. **The other kind of file, and it is defended differently because it is a different thing.** Since M67 `POST /api/v1/addons` takes a `manifest` part and a `module` part and installs an add-on into a running instance. **Nothing about those parts is sniffed, and that is not the rule above relaxed.** A logo is bytes this product decodes and then serves back to a browser, so what they *are* has to be settled before anything touches them. A module is bytes this product *executes*, and what settles those is stronger than a leading-byte check: the module is hashed and compared against the `sha256` the manifest declares **before it is written anywhere**, the manifest is parsed by the same reader the boot path uses, and bytes that are not a WebAssembly module this build can serve fail to compile and are refused with nothing left on disk. A magic-number check would admit a superset of that and would still say nothing about whether the module is the one somebody reviewed. **Since M68.6 the bytes may also arrive over a fetch this instance makes**, and that is a change to who chose them: the paragraph here used to say nothing is fetched from a URL, and what replaces it is not a relaxation but two bounds and a digest. The address is checked **after resolution, at dial time, on every address a name resolves to and on every hop of a redirect**, against the same globally-routable-unicast allowlist an add-on's own fetch meets — so a URL install cannot be pointed at loopback, at link-local, at the cloud metadata services, at an RFC 1918 range or at anything nobody has thought about, because refusal is the default. A redirect that leaves the origin that was typed is not followed. And the `sha256` is **the operator's**, never the URL's: it covers the whole fetched bundle, it is checked before an archive reader, a decompressor or a JSON parser is pointed at the bytes, and nothing is written unless it matches. That ordering is what keeps the container parsers from being reachable by anybody who can serve a response: the bytes have to be the bytes the operator named before one of them runs. A bundle is a `.tar`, a `.tar.gz` or a `.zip` holding `addon.json` and the module it names, in one object, so a manifest fetched from a URL can never be the only source of its own module's digest — the alternative, fetching the two separately, is what would make that possible and is refused. **Which of the three it is comes from the file's leading bytes and never from the URL's extension**, because the extension is typed by whoever supplied the address and is therefore a claim rather than a fact. **All three carry one member rule and it is the depth limit**: exactly two plain files with bare names — no directory entry, no symbolic link, no hard link, no device, no path component, no `..`, no duplicate name — which is depth zero, asserted per format by test so the tar path and the zip path cannot drift into two policies. A zip's own defects are refused by name: a duplicate member, a name carrying a separator, and any member that is not stored or deflated. **The bound is on the body and it is a number**: 32 MiB over the whole multipart envelope by `http.MaxBytesReader`, and the same 32 MiB over a fetched bundle, so neither shape can send two 30 MiB pieces and have each pass a check of its own. **A compressed bundle is bounded on the wire and again on what it may amount to once opened — with one gap named in LNK-F-0340: the HTTP transport may itself have inflated a `Content-Encoding: gzip` response before either bound sees it, so *what was fetched* is the decoded figure**: the fetched bytes as above, and the decompressed bytes at *the smaller of* 32 MiB and fifty times what was fetched. **That figure is where the decompressor is stopped, not a verdict passed on what it produced**, and the difference is the whole of the second bound: a limit evaluated after inflating has already bought the allocation and the CPU it existed to refuse, so a two-kilobyte archive would still spend the whole budget for the price of one small request and merely be told afterwards. Reading stops one byte past what could have been accepted, which for a small request is a small number; a zip reaches the same figure from the other side, at its central directory, before a byte is inflated at all. The ratio does not apply below a mebibyte, where a tar is mostly padding and the figure says nothing about the archive; this project's own modules compress by three to four. Accepting compression is therefore a defence this product takes on deliberately rather than one it avoids by refusing the format; the member rule is what makes that affordable, since a bomb still has to present exactly two bare-named plain files. **What no mechanism here can do is make the operator's digest mean something**: a URL and a digest copied off the same web page authenticate nothing, the install form says so where somebody is about to paste them, and publisher identity is what would answer it rather than anything this milestone ships. **One bound an add-on's own fetch has is deliberately *not* inherited here, and it is worth stating rather than discovering**: an add-on may reach only the origins its operator wrote into a setting, and an install has no such allowlist because the act *is* the authorization — somebody holding `addons.manage` typed an address at that moment. What authorizes the destination is therefore the permission and the typing, and what bounds it is the address policy and the digest, which is why those two are stated at length above and why an origin allowlist would have meant nothing here. It follows that this door is only as good as the permission: `addons.manage` is an instance-level scope no API key can hold, and anybody holding it can already put files in the add-ons directory. Both parts are read whole into memory rather than streamed to disk, and that is chosen rather than inherited — the module is hashed before it is written, so the bytes are held either way, and streaming first would mean putting an unverified module into the directory this instance executes from. **What gates this address is not the file check.** It is `addons.manage`, an instance-level scope no API key can hold, and every install and every removal is in the instance-wide audit log naming the module, its version and its digest. What that permission asks an operator to decide is in *Operator responsibilities* below. |
...
| Egress | No telemetry, and no third-party calls that were not agreed to by somebody administering this instance. GeoIP is a local file. **This row said *no phone-home in the default configuration* until 0.3.0, and that sentence is edited rather than qualified** — the update check below is a phone-home by any honest reading, it is offered by default (D149), and it does nothing at all until an operator has been asked and said yes (D164). **Seven** connections leave this product, enumerated below rather than counted, because this row said *two* until M45 and both of the missing ones were shipped features. What is being counted is *a socket this process opens to a host outside its own deployment*, and the seven divide by who decides them. **The sixth arrived in 0.4.0 and is not like the other five**: an **add-on's outbound request**, made by code this project did not write, to an origin the operator typed into that add-on's settings. Every other entry here has a destination this repository chose or an operator configured for a purpose this product defines; this one is a capability granted to somebody else's module — see *Add-on egress* below, which is where the whole of what bounds it is written. It is off for every add-on until an origin is named, and an instance running no add-on that declared `network.fetch` never opens it at all. **The seventh arrived in 0.4.0 as well and is the only one nothing configures**: a **URL install** fetching an add-on bundle. It is not a setting and not a schedule — it happens once, when somebody holding `addons.manage` types an address and presses a button, to that address and no other, and an instance whose add-ons are uploaded or placed by hand never opens it. It is counted here anyway, because what is being counted is a socket to a host outside the deployment and *an operator asked for it* is a description of who decided rather than an exemption from the enumeration. It shares its whole mechanism with the sixth — the same address policy, the same redirect rule, the same one file — and the uploaded-content row above is where the digest that bounds it is argued. **The operator's three. Two are off until they configure one:** an SMTP relay (`SMTP_HOST`), and a reputation feed (`FEED_URL`) which sends the destinations your users type to a third party — the consequential one, and the one that discloses itself, see *Reputation feeds* below. **The third is the only one an instance turns on by agreeing to a question rather than by being configured:** a daily `GET` to GitHub's releases API asking whether a newer LinkCtrl exists. It carries this server's source address and the running version in the `User-Agent`, and **nothing else** — no instance identifier, no deployment size, no link counts, no configuration, no request body, no query string, no credential; the response is read for a version and discarded. **It is off until somebody is asked and says yes** — on the setup form for a fresh instance, on the dashboard at the first sign-in by an account holding `instance.admin` for one that was upgraded — so an instance nobody administers never makes the request at all. And it is refused three ways, any of which is enough: `LINKCTRL_UPDATE_CHECK=false` on the deployment, *no* at whichever prompt was shown, or simply never answering. The destination is a compile-time constant, not a setting, so there is no value anywhere that redirects it somewhere else. A test compares the outgoing request against an exact expected form and fails when a field is added to it. *No notification is not evidence of up to date*: a throttled or blocked check does nothing and says so only at debug. **A workspace's two, and no operator setting turns either off:** a webhook delivery to a URL an owner or administrator registered, carrying that workspace's link events and the destinations in them (*Outbound webhooks* above); and a DNS `TXT` query for the challenge label under each hostname a workspace has registered, made when it is registered and hourly thereafter (*Custom-domain re-verification* above). **The DNS query is the weakest of the seven and is counted anyway.** The socket goes to *this host's own resolver*, the one the operator configured; it is the query rather than the connection that reaches a nameserver the registrant chose, and what that nameserver observes is this instance asking about a name it was given, on a clock. Excluding it would rest the number on which end of a resolver you stand at — and a count arrived at that way is the count that was wrong before. It is also why *Outbound webhooks* can still say webhooks are the one place this product connects to an **address** a user chose: they are, and neither DNS nor the update check is a counter-example to it — the update check's destination is a constant in this repository's source. **Not counted, so the seven can be checked:** Postgres and Redis, which are the deployment rather than somewhere outside it, and `linkctrl healthcheck`, which dials this process's own listener. Nothing else opens a socket outwards, and a source scan over the package that judges destinations fails on any outbound-HTTP or name-resolution symbol, so a later "just check the host resolves" cannot become undisclosed egress. That scan is untouched by the update check, which lives in `internal/update` and is not visible to it, so nothing was added to its allowlist to let this land. **Since 0.3.0 a second scan covers the authentication path** — `internal/auth` and `internal/httpx`, in the same shape and with the same symbol list. It exists because the second factor is exactly the kind of feature that arrives with a verification service attached: TOTP is arithmetic over a clock and a shared secret, so there is nobody to ask, and the count above did not move when it landed. The two scans are separate because the first reads its own directory and could not have seen either of these packages, which is a gap the milestone found in its own draft rather than in the tree. **0.4.0 gave that second scan its first exemption, by name and with an argument**: `internal/addon/fetch.go` dials, because an add-on may reach outward and an OIDC add-on will do so on an authentication path. The sentence the scan asserts narrowed accordingly and is stated rather than quietly widened — *this product's own authentication code does not reach the network* — and it holds only because the egress is confined to that one file, which is what the exemption asserts. A second file in `internal/addon` that dials is a second door with its own bounds and fails the scan rather than joining a list. **The seventh connection joined without a second exemption**, which is the check working: a URL install builds a request and hands it to that same file, so the dialling verbs stayed where they were and the scan is still asserting one door rather than a list. |
| Add-on loading | **Off unless `LINKCTRL_ADDONS_DIR` is set**, and off means no WASM runtime is constructed at all. When it is set, every module is verified against the `sha256` in its manifest **before it is compiled**, so a module whose bytes are not the bytes the manifest describes is never parsed, let alone instantiated. The manifest's `module` field must be a bare filename and a separator or a `..` is **refused rather than cleaned**, so a manifest cannot name a path outside its own directory. Unknown manifest fields are refused, because `schema_version` is checked for equality and a field this host does not know means a file written for a schema it does not implement. **The manifest is the trust root and nothing hashes it**, so the host is held to reading it the way a person does: a key must be spelled exactly as documented and must appear once, at every level of the document. That is a boundary rather than strictness, and it was measured — a JSON parser keeps the **last** occurrence of a repeated key and binds a case-variant spelling to the field anyway, so `{"permissions": ["session.mint"], "permissions": []}` loaded with no grants and the same pair in the other order loaded with all of them, while `"SCHEMA_VERSION": 7` beside `"schema_version": 1` loaded as schema 1. A publisher could write a file whose readable text said one thing to a reviewer and another to the host; both spellings are now refused and the error names the documented one. A module is instantiated with **no capability granted**: no filesystem is preopened, no environment or arguments are passed, and its stdout and stderr are discarded. **The clock and the random source are the exception, and they are now this machine's rather than the runtime's fakes.** The runtime defaults to a compile-time-constant random source and a clock starting at 2022-01-01, and this instance was shipping both — so every add-on on every deployment drew the same bytes and, with a fresh module instance per request, every visitor was handed the same value. Handing an authentication add-on a fixed nonce is a worse failure than any capability withheld by leaving them fake, so both are wired to the operating system's: a module's own `crypto/rand` and `time.Now` read this machine, and `random_bytes` and `time_now` in the ABI are the same two sources with a documented shape. Neither costs a permission, because gating one would refuse an add-on the documented spelling of something it can still have. The imports it may resolve are the **published ABI** and WASI preview 1, whose other capabilities the host leaves empty — no preopened file, no environment, no arguments, discarded output, no socket. What an add-on will not load means is the **add-on's** declaration and not the operator's: `required` stops the instance with the reason, `degrade` lets it serve without the module, and a manifest that cannot be parsed stops the instance too — there is no class to honour, and assuming the forgiving one would boot an instance with an authentication add-on silently missing. **A module that never finishes loading is a failure like any other**, which it was not until this was measured: package initialization runs during instantiation, so a module that loops there returns nothing, and with no deadline on the context the load ran under the instance never reached its listener — no log line, no metric, no error, and an operator's only remedy a second signal. Compiling and starting each add-on's module are now bounded at 30 seconds **each**, so one add-on's guest execution is at most 60 seconds and an instance's bring-up window is that times the number of add-ons that hang; expiry is counted as `linkctrl_addon_loads_total{outcome="load_timeout"}` and named in the log, and the class the manifest declared decides what happens next. Starting is interrupted wherever it has got to; **compiling is checked between the module's functions**, so a single function that is pathologically slow to compile overshoots by however long that one function takes — the bound is real and its granularity is stated rather than rounded up. The budget is **per add-on** rather than for the directory, because a shared one, once spent, would refuse every add-on behind the slow one — turning a `degrade` module's hang into a `required` module's refusal to boot. It bounds the add-on's own code and not the host's work with the database: an add-on's migrations wait on the migration lock for as long as this product's own do, which is what stops the bound turning a replica that arrives mid-migration into a crash loop. |
| Add-on ABI | **An add-on reaches this product through an enumerated set of imports and through nothing else** — no socket, no file, no shared table, no environment — so the whole of what an add-on can do is one list, in `internal/addon/abi`, published as [addon-abi.md](addon-abi.md) and as a generated SDK. Sixteen of those functions do anything today; the rest are declared and answer a refusal, so the contract is complete before the behaviour is. **Both numbers on this row are counted rather than kept here** — the first said eight through the milestone that made it twelve, so `TestTheDocumentedLiveCountIsTheOneThisListHolds` reads the list and holds these two sentences to it. **No function hands a module a client's address in any form.** That is a property of the surface rather than a promise about add-on code: the record carrying redirect data is bound to what `click_events` may carry — country-level, and no address column exists to carry — and a test reads the column list out of the migration to prove the bound, because this project cannot review the source of an add-on it did not write. **What that bounds is the host, not the module** — this row said *an add-on cannot store what it is never handed* until 0.4.0, and it does not follow: an add-on holding `routes.own_prefix` writes its own `Location`, so it can send a visitor to an origin its author controls, observe the address there, and correlate it back through a cookie under its own declared prefix, with `storage.own_schema` to hold the result. Nothing here stops that and nothing could — installing a module is a trust decision an operator makes, and the row two below already states redirecting a visitor anywhere as an accepted consequence of it. Region and city are refused as well, though the table has both columns: they resolve transiently and are never stored, and a module with a schema of its own is exactly where they would stop being transient. **No function hands a module a credential of this instance's either**, and that is the same kind of property: sessions here are server-side and opaque, so the `Cookie` header *is* the session, and an add-on serving a route is handed only the cookies whose names begin with one of the `cookie_prefixes` its manifest declares — each of which must begin with the add-on's own name, so no prefix an add-on may declare reaches `linkctrl_session` in either spelling. It bounds what an add-on may *set* as well, because a cookie it may not read is one it must not overwrite. **Beginning with its own name does not on its own keep one add-on out of another's namespace**, and that was measured rather than reasoned about: add-on `oidc` may declare the prefix `oidc_x`, which begins with `oidc_` and is a prefix of every prefix add-on `oidc_x` is allowed to declare, so with both installed the first read and overwrote the second's state cookie. The same relation makes `LINKCTRL_ADDON_OIDC_X_KEY` two add-ons' setting at once. So the rule has a second half at load: two installed add-ons whose names stand in a `name + "_"` prefix relation are **both** refused — neither is awarded the other's namespace and neither wins by loading first — counted as `linkctrl_addon_loads_total{outcome="name_collision"}`, with the pair named in the boot log and the instance stopping if either is `required`. The route prefix and the Postgres schema were never exposed to it: each uses the whole name as one segment, with nothing joined onto it. The one payload the host itself composes — what comes back when a module's authentication claim is accepted — is enumerated for the same reason a list of forbidden names is not enough on its own: it carries when the session expires and whether a second factor is still owed, and no token, no cookie and no row of the sessions table, so the release that implements it cannot quietly widen it without the change being visible on the published surface. An add-on that genuinely needs some other cookie of the host's cannot have it, which is accepted: the alternative is a trust model in which an installed add-on can impersonate any signed-in user, and that would have to be stated here rather than implied. A single value crossing into the host is bounded at 64 KiB, so a length is not a way to make this process allocate a gigabyte, and a module's only route to the operator's log is the `log` function, attributed to the add-on. What the ABI does **not** hand over is also worth knowing: no socket, no file, no environment, and no reach into another add-on's schema. **Narrower still inside a redirect.** An add-on running on the redirect path holds a visitor's request open, so an invocation there reaches only a named subset of the list — the ungated host facts, its own settings, and the two functions the class exists for — and everything else is refused **whatever its manifest declared**. Storage is the one that matters and is why the subset is enumerated rather than described: an add-on that owns tables is exactly the add-on that would write to them from the hot path. That is the redirect tree's own *no session lookup, no CSRF, no template rendering* rule reaching across the boundary, enforced at the call. A clock and a random source it *does* hand over, ungated — `time_now` and `random_bytes`, which M65 added because an authentication flow needs a `state` value, a PKCE verifier and an expiry to compare against. Neither is a grant, and that is a statement about what gating would buy rather than about how harmless they are: a module already reaches both from inside its own runtime, since the host wires `crypto/rand` to the same reader and `time.Now` to the same clock, so a permission on either would cost every manifest a line and stop nothing. |
| Add-on permissions | **What an add-on may call is what its manifest declared, and the host enforces it rather than trusting the module.** Every function in the ABI names the permission it costs; the host resolves a manifest's declarations against a **closed** nine-token vocabulary at load and refuses any call whose grant is not held, with a status the module can branch on and a `linkctrl_addon_refusals_total{addon,permission}` counter an operator can alert on. The check is in the host's dispatch and not in a function's body, so a capability cannot arrive with its check somewhere else, and it runs **before** the host says whether it implements the function at all — an add-on that declared nothing cannot use the ABI's availability status to enumerate what this build can do. A token outside the vocabulary refuses the add-on at load, for the reason an unknown manifest field does. A token the vocabulary carries and this build grants to nobody is declarable, is never held, and every call behind it is refused; the boot log names it and `linkctrl_addon_info` does not, because that label carries what a module holds rather than what it asked for. **There is no such token today**, and there was one until an add-on could run inside the redirect path: `redirect.inline` was declarable and ungrantable for four milestones so that the milestone which admitted an add-on onto that path enforced behaviour against a permission already being enforced. **Three of the nine are the redirect path's and they are a ladder rather than a set**: `redirect.observe` watches redirects out of band and can affect none; `redirect.inline` runs on the path and may refuse a redirect outright; `redirect.rewrite_query` lets an inline module alter the destination's query string and nothing else about it. The third is a token of its own rather than something the second implies, because a manifest declaring *run on the redirect path* should not turn out to have declared *and edit where the visitor goes* — the same argument that keeps the first two apart. What bounds the rewrite is not inspection: the module writes a query, and the host substitutes it into the URL **it** decided, so the scheme, the host, the port and the path cannot move and no tier of the destination validator — every one of which judges by host — can reach a different verdict. An add-on is therefore not a second way around that validator, and the host asserts the substituted URL differs in its query and in nothing else rather than trusting the argument. Four functions cost nothing and are ungated deliberately: reporting the ABI version, writing a line to the log — the one capability granted on purpose — and drawing random bytes or reading the clock, which are ungated because a module reads the same two sources through `crypto/rand` and `time.Now` anyway, so a grant on either would withhold nothing and cost every manifest a line. **Ungated is not trusted**: because every loaded module can reach the log, including one that declared nothing, the host neutralizes the message before the line is written and bounds it at 4 KiB. **The claim is about the module and so is the neutralization**, which is a correction: `log` is not the only path from an add-on's bytes to an operator's screen. Manifest validation embeds the value it refused with Go's `%q`, and `%q` escapes on `unicode.IsPrint` — true of every mark and every letter — so `U+3164 HANGUL FILLER`, `U+FE0F`, `U+E0100` and `U+2D7F` passed through it untouched, and a migration filename admits every code point but a newline. **It is enforced by the logger rather than by a rule about which call sites to write carefully**, because that rule was written down twice and enumerated wrongly twice: the second list named three sites and missed the line `internal/store` writes when a migration is **applied** — the path that runs when nothing is wrong, in a package that cannot import the neutralizer — and the per-request instantiation failure, whose text wazero builds out of the module's own name section and which a guest can arrange for the per-request instance alone. The host now wraps the logger it is given, and every logger anything in the add-on subsystem uses is derived from it, including the one handed to `internal/store`; so a log call added later, in a file nobody has thought of, is neutralized without its author knowing this boundary exists. There is no site to miss and therefore no list to get wrong. Errors leaving the subsystem are neutralized the same way, once, at the exit rather than at each of the places one is built, and `errors.Is` still reaches the sentinel underneath. So a module can neither forge a record that reads as this product's own nor put a **default-ignorable** character in front of a reader. That second claim is deliberately narrower than the one it replaces — this row said *nor arrange bytes in an operator's log to be overlooked* — and narrower still than *a module cannot put invisible bytes in an operator's log*, which a rejected attempt at this fix substituted for it and which never shipped. Both are false: seventeen `Zs` code points survive as themselves and `U+00A0` is pixel-identical to a space, thirteen `Cf` survive by the allowlist this same cell names by hand further down — and `U+00C5` and `U+212B` are canonically equivalent and render the same. No escaping rule that keeps text can support an absolute claim, so the claim made is the one that can be checked: every member of Unicode's `Default_Ignorable_Code_Point` is either deleted or shown as its code point, and a test walks the whole range asserting it. **Nor is it a redaction pass**: `log` costs no permission, so an add-on that wants a secret in an operator's log writes one in plain text. What is defended is legibility — that what a reader sees is what is there — and not confidentiality, which this boundary was never in a position to offer. **The neutralization is stated as what survives, not as a list of what is caught** — a **graphic** character reaches the line as itself, meaning every letter, mark, digit, punctuation mark, symbol and space in every script — with the two exceptions named below — and everything else becomes its escape: a newline, a control character, an ANSI escape, every format and bidirectional control, every unassigned or private-use code point, and the 268 graphic code points the host treats as invisible. That last set is the 267 graphic members of Unicode's `Default_Ignorable_Code_Point` plus `U+2800 BRAILLE PATTERN BLANK`, which the property does not carry and which is escaped for a reason of its own — it is the one blank that is not whitespace, so nothing that trims or splits treats it as one. **It is a published property and not the set of characters that render as nothing, and the difference is stated rather than papered over**: Unicode publishes no property for the second, four attempts at this milestone were spent discovering it, and the residue is named here. Eight combining marks the UCD annotates as *"shape shown is arbitrary and is not visibly rendered"* are outside `Default_Ignorable_Code_Point` and reach a line as themselves — `U+2D7F`, `U+17D2`, `U+10A3F`, `U+1107F`, `U+11A47`, `U+11A99`, `U+11F42` and `U+16FE4` — and so do the seventeen `Zs` and the thirteen prepended concatenation marks this row already names. They are not added to a list: a hand-written enumeration of invisible characters is behind the next Unicode revision by construction, which is the mistake `Other_DI` was the third form of. **What bounds the residue is that the log is write-only to a module** — an add-on may *post* to it and may not *read* it, so a value accidentally logged cannot be retrieved by the module that wants it. `log` takes a level and a message and declares no out-parameter; no function in the ABI hands log content back; a guest gets no preopened file and its stdout and stderr are discarded; and an add-on's storage is a schema of its own that this log does not live in. **All four are asserted and none of them rests on a configuration nobody is watching**: the middle two used to rest on wazero's defaults at the two instantiation sites, so nothing failed if a later change added a filesystem or routed a stream somewhere, and they are now asserted from inside a guest — a fixture that lists `/`, opens a host file and opens its own manifest, and fails instantiation if any of the three succeeds, plus a marker written to each stream that must reach neither an operator's log nor the host process's own two. So a character that survives is one an operator can still see, and the channel pays out only if an operator hands the log to the add-on's author — a person's decision rather than a capability this host grants. `TestAnAddonPostsToTheLogAndCannotReadItBack` drives every function in the ABI to assert it, and `TestTheResidualClassIsWhatTheDocumentsConcede` pins the eight. **The host derives that property rather than asking for it** — `Other_DI` union `Cf` union `Variation_Selector`, less `White_Space`, `U+FFF9`–`U+FFFB`, `U+13430`–`U+13440` and the prepended concatenation marks, all **seven** terms of the derivation Unicode publishes — because Go ships no table under that name and the first form of this asked for `Other_Default_Ignorable_Code_Point` instead, which is the *residue* the derivation subtracts from and carries 3776 members of which 7 are graphic against the real property's 4174 and 267 (F285, D283, D284) — and 4174 is what Unicode 15.0's own file totals for the property, which is what makes the derivation checkable rather than merely stated. The first fix wrote six of the seven terms, dropping the Egyptian hieroglyph format characters, and claimed 4190; the sixteen it over-counted are format characters that were escaped either way, so what was wrong was the sentence and not the boundary, and every count in this row is now pinned by an **equality** in `TestNoDefaultIgnorableCharacterReachesALogLine` rather than by a floor. The derivation reaches 398 members the residue lacks; the 260 of them anybody could see are the **variation selectors**, every one of them category `Mn` and so graphic, and a module declaring no permission at all used them to carry `SECRET=hunter2` out of a line reading *everything is fine* — reproduced twice, under both handlers, with a nibble scheme and a byte-per-selector scheme. **Those 260 are deleted rather than escaped, and they are the one class this boundary removes**: a selector has no appearance of its own to report, so escaping it would put `\ufe0f` through the middle of every emoji anybody logs and buy a reader nothing, while deleting it costs a reader nothing — `❤️` becomes `❤`, still a heart, and `😀` is untouched. **There is no exemption for a legitimate emoji case and no list of bases**, and that is the correction M62 took two rejected attempts to reach: one keyed the exemption on category `So`, and a progress bar drawn from `█` and `░` — 6304 of the 6634 symbols carry no registered sequence — went through it byte for byte carrying the secret; 6634 is the size of the category and is pinned by test with every other number in this row, while 6304 is **attributed rather than pinned**, being uncomputable from the tables Go ships and measured on 2026-08-20 against the UCD's `emoji-variation-sequences.txt` on the branch that vendored it; the second keyed it on Unicode's own registered sequences, read from the UCD's data file, and was defeated in turn by building the channel out of registered bases whose selector a renderer ignores. Whether a selector is visible depends on the font in front of the reader, and no property names that set. Both attempts are on a parked branch and neither shipped. That inversion is deliberate and it replaced an enumeration (D242). No release ever carried the enumerated form this replaced, and it was already incomplete when written — it missed `U+061C ARABIC LETTER MARK`, an invisible bidirectional control from the same Unicode revision as the isolates it did cover, along with the interlinear annotation controls that hide the run they wrap; a list of invisible characters is behind the next revision by construction, so it cannot support a claim like this one. **The exceptions now run the other way and are named**: Unicode's `Prepended_Concatenation_Mark` property — `U+0600`–`U+0605`, `U+06DD`, `U+070F`, `U+0890`, `U+0891`, `U+08E2`, `U+110BD` and `U+110CD` today, the Arabic, Syriac and Kaithi signs that scope the digits following them — is left alone, because a boundary that mangles Arabic is a worse defect than the one it prevents. **It is read from Unicode's own property and not transcribed from it** (D243): a hand-copied form, also never released, carried eleven of the property's thirteen members, so `U+0890 ARABIC POUND MARK` and `U+0891 ARABIC PIASTRE MARK` were escaped from the day it was written — the same staleness the enumeration had, in the allowlist that replaced it. **One graphic character is escaped the other way**: a backslash is doubled (D244), because the two characters `\` and `n` otherwise reached the line as the bytes an escaped newline reached it as, and because that is what makes the truncation mark `…\(truncated)` something a module cannot spell — a log read as evidence may not leave a reader unable to tell what was written from what the host did to it. **What the inversion costs** is legibility rather than safety, and it is stated because it is real: a code point assigned by a Unicode revision newer than the host's Go toolchain is not a graphic character to that host, so it is escaped until the host is rebuilt. The set is asserted from Unicode's own categories by test rather than from a second copy of the list. It is done in a `slog.Handler` the **host** wraps its own logger in — not in the logger an operator configured, which is not something this boundary may depend on, and not at each call site, which is the rule that was enumerated wrongly twice. It refuses nothing: a message that needed neutralizing is still logged. Off the `log` path the same escaping is applied once, because applying it twice would double a backslash twice: a value goes to the logger raw and the wrapper escapes it, and the two kinds of error whose text must already be neutralized because they are *also* printed outside a log carry a marker that says so, at which point the wrapper only folds and bounds them. **The bound is the log line's and belongs to nothing else.** Manifest validation reports every problem at once, so that somebody publishing an add-on for the first time sees the whole list; fusing the escaping with the 4 KiB cap cut that list silently and ran it onto one line, which is M60's stated behaviour regressed as a side effect. The two are separate now, and the same failure still occupies exactly one bounded log record. **A grant is held, never inferred** — nothing about a module's name, its code, its other declarations or its install order widens it — and it is conferred by the operator's act of installing a module whose manifest asks for it, not by a role. That makes it a parallel to `NonDelegableScopes` rather than a use of it: the two are separate mechanisms over separate principals, and the reasoning is in Mustur's LNK decisions. |
| Add-on storage | **An add-on that asked for tables gets a Postgres schema of its own and a database role that reaches none of this product's data.** *Reaches nothing else* is what this row said until the two paragraphs at its end, which are there because both were measured rather than reasoned about: a confined role can still create a **large object** and can still take one of this product's **advisory locks**, and neither is a read of a product row. The schema is `addon_<name>` and the role has the same name, both derived from the add-on's name rather than recorded anywhere, so two add-ons cannot contend for one schema — the directory *is* the name, and a manifest that disagrees with its directory is refused. **The boundary is privileges, not the search path**, and the distinction is the whole of it: a search path decides where an *unqualified* name resolves and is never consulted for `public.links`, so it confines nothing on its own. The role holds no privilege on any table outside its own schema, so a schema-qualified read of this product's tables is refused by Postgres. **Privileges only bind if the session is that role**, which is why the host opens a second connection pool per add-on, authenticated as the add-on's role with a credential generated at every boot and stored nowhere. **A second replica booting rotates that credential**, since each mints its own, so a replica whose credential has been rotated out from under it mints a new one on its next connection and logs that it did — without which every replica but the last to boot would lose add-on storage quietly, the failure being a connection refusal and not a privilege one. `SET ROLE` on the application's own connection was tried, measured against Postgres 17 and rejected as a boundary: a single `DO $$ BEGIN EXECUTE 'RESET ROLE'; … END $$` escapes it, and `SET SESSION AUTHORIZATION` is checked against the *session* user rather than the current role, so it succeeds whenever the application connects as a superuser — which the shipped compose file does. Authenticated as the role, both are refused. One statement per call, because the host parses through Postgres's extended protocol; a read runs in a `READ ONLY` transaction, so the ABI's read function cannot write; the search path and a five-second statement timeout are pinned per transaction, so nothing a previous statement left on a pooled connection changes what the next one means. **Migrations are the host's and are third-party DDL the operator did not write**, and two things bound that together: the manifest names every `.sql` file with its digest, so the DDL that runs is the *add-on author's* rather than whatever is on disk — a file the manifest does not list refuses the add-on, and so does one whose bytes disagree — and the migrations are applied *as the add-on's own role*, so DDL naming another schema is refused by the database and a `SECURITY DEFINER` function the DDL creates is owned by a role that can reach nothing. The host then asks Postgres itself four questions and refuses the add-on if any answer is not empty: what does this role own that is not in its own schema, what is in its schema that this role does not own, and what has it granted on that schema to anybody but itself. The first two are each a set difference over the catalogue Postgres's own `DROP` consults — `pg_shdepend` for `DROP OWNED BY`, `pg_depend` for `DROP SCHEMA` — and asking a shape rather than checking a list of places is deliberate: three earlier versions of this check enumerated the places an add-on might own something and each was missing one, the last being a temporary relation that the exclusion added for TOAST also hid. **The third question is there because an add-on can give its own schema away and this document once said it could not**: a grant is not an object, so neither catalogue above records one, and two ordinary statements from the confined role — `GRANT USAGE ON SCHEMA … TO PUBLIC` and a `GRANT SELECT` beside it — let another add-on read those tables while both set differences stayed empty (measured; D255). Nothing prevents it, because ownership carries the right to grant and Postgres offers no way to withhold that from an owner; what the host does is read `pg_namespace.nspacl` and `pg_class.relacl` at every load, name any grantee that is not the add-on's own role, and refuse to run the module until it is revoked. The harm is bounded to what the granting add-on chose to give, which is its own data. It cuts the other way too: a grant *you* made — a reporting role with `SELECT` on an add-on's schema — stops that add-on loading, and [operations.md](operations.md) carries the remedy. The second question exists because `pg_dump` carries no roles, so a restore into a cluster whose roles were not restored separately leaves an add-on's tables owned by the application, which no amount of asking what the *role* owns will find. An add-on's migration state is a goose table inside its own schema, so a re-load applies nothing twice and an add-on's data has no half in a table this product owns. **Nothing caps how large that schema may grow** — the same answer the audit log gets — and `linkctrl_addon_schema_bytes{addon}` is what makes the *stored* growth visible. **That gauge kept a denylist of its own until 2026-08-19**, which is worth stating here because it is the same defect as the enumeration above, left standing one function away from where the argument against it was made: it summed ordinary and materialized tables, so a **sequence** — `relkind 'S'`, in the add-on's own schema, 8192 bytes from creation, and outside `pg_total_relation_size` of the table that owns it — was not counted. 24,000 of them moved `pg_database_size` by 188 MB with the gauge reading 0. It now sums every relation kind in the schema except the ones already counted inside another, measured as an identity against a second decomposition of the same bytes. Nor was it only an adversary's case: the migration table the host itself creates in that schema declares an identity column, so every storage add-on has always held 8192 bytes this number reported as nothing. **What the post-condition and the two gauges between them cover is every kind of object Postgres catalogues, which is not the same as every way an add-on can use disk**, and the qualifier is stated because the argument for asking a shape rather than keeping a list would otherwise be overclaimed: a `WITH HOLD` cursor materialized at commit holds a temporary **file** for the life of the session — 553 MB measured for one cursor inside the five-second statement timeout — and it is an object in neither catalogue, so the post-condition is empty and both gauges read zero while it sits on disk. It is transient rather than stored data, which is why nothing here reports it and why it is a residual rather than a hole in the confinement; the bound that would close it, `temp_file_limit` on the add-on's role, needs a superuser to set, which is the same conditionality as the two narrowings below. **Two things the role can put outside its schema are accounted for rather than prevented, and it is the post-condition above that accounts for them** — a list of them here would be the same denylist the check stopped keeping. A **large object** is the first: `EXECUTE` on `lo_from_bytea` belongs to `PUBLIC`, Postgres has no per-role deny, and revoking it needs ownership of a `pg_catalog` function — which the application's role does not have, and where the role is not superuser the `REVOKE` is a **silent no-op** rather than an error, so shipping one would be a control that does nothing on the deployments this product documents. 40 MB in a single statement was measured, from the write function only; the read function's `READ ONLY` transaction refuses it. What holds instead: `linkctrl_addon_large_objects{addon}` publishes the count, the load post-condition refuses an add-on that owns one, and `docs/operations.md`'s purge carries the `DROP OWNED BY` that drops it — the schema drop alone does not, and `DROP ROLE` then fails. A **temporary table** is the second, and it is *narrowed* rather than only accounted for: `PUBLIC` holds `TEMPORARY` on a database by default, a pooled connection is not a fresh session so the relation survives across calls, and 51 MB went into one inside a single five-second statement. Installing a storage add-on now revokes `TEMPORARY` on the database from `PUBLIC` and grants it back to the application, after which `CREATE TEMP TABLE`, `CREATE TABLE pg_temp.x`, `CREATE TEMPORARY TABLE … AS`, `CREATE UNLOGGED TABLE pg_temp.x` and `SELECT … INTO TEMP` are each refused — measured, all five. **That revoke is a narrowing and not a boundary**, for three measured reasons: it does nothing but print a warning unless the application owns the database, which this document asks for `CREATEROLE` and not ownership; no dump carries a database privilege, so it is gone after any restore; and it changes the database for every role, so an operator sharing this database with another application that uses temporary tables loses them — [deployment.md](deployment.md) states that. A per-role revoke is not available, because the privilege arrives through `PUBLIC` and Postgres has no per-role deny. **A third narrowing is the only one in this family conditional on neither superuser nor database ownership, and it is why the family is worth naming as one**: the confining role may `ALTER ROLE CURRENT_USER SET` any user-settable parameter on itself — `work_mem = '4GB'` was accepted, landed in `rolconfig`, and a fresh connection read it back, so one read inside the five-second timeout peaked a backend at 1.37 GB resident against 31 MB for the same query at the 4 MB default, which spills to disk instead — and there is no per-role deny for that either. So every load now clears the role's settings before pinning its search path, which needs only the `CREATEROLE` this document already asks for and survives a restore, unlike the two above. **That sentence was short by one scope for a phase, and the correction is the interesting part**: `ALTER ROLE … RESET ALL` clears the role's *cluster-wide* defaults, and Postgres keeps a second set per role **per database** that the same role writes with `ALTER ROLE CURRENT_USER IN DATABASE <db> SET`. Those survived every load — measured, a fresh login reading `work_mem` back as 4 GB from `src = database user`, a confined sort of 400,000 rows then running fully in memory where the 4 MB default spills to disk. Scoping a second reset to the current database is not the repair, because `IN DATABASE` takes a *name* and not a connection: the confined role's write is accepted for a database with `CONNECT` revoked from `PUBLIC` and for `template0`, which allows no connections at all. The load therefore reads the databases out of `pg_db_role_setting`, which is shared across the cluster, and resets each one it finds before resetting the cluster-wide row — so there is no database left to name. The post-condition asks that catalogue too and reports any row but the search-path pin the load itself writes, in any database, which is what would catch the repair failing later; a setting is not an object, so the two set differences above are blind to one by construction, exactly as they are to a grant. **What is repaired is repaired per add-on, at that add-on's load, and nothing sweeps**, which leaves one residue stated rather than absorbed: nothing in this product drops an add-on's role — the only `DROP ROLE` is the purge an operator runs from [operations.md](operations.md) — so a role whose module was deleted keeps whatever it parked, with no load left to clear it. A boot-time sweep over roles no add-on claims was built and removed. It identified its targets by the `addon_` name prefix, and a reviewer falsified that with six words: `CREATE ROLE addon_reporting` — `NOLOGIN`, no schema, no module, never made by this product — carried settings in two scopes and the sweep took both, which is this product mutating a cluster-shared catalogue over a string. The obvious repair, asking the catalogue for the two facts only this product's installer writes — a schema of that name in this database that the role owns, and a `pg_auth_members` row making the application's own database user a member — does not hold either, and the measurement is the reason: on Postgres 17.10 a `NOSUPERUSER CREATEROLE` role, which is exactly the privilege this document asks for, gets that membership row *automatically* with `admin_option` from a bare `CREATE ROLE addon_x NOLOGIN` with no `GRANT` typed, so the fact proves nothing on the deployment shape it was meant to protect. What holds instead needs no proof of ownership at all, because it repairs nothing: the post-condition above reads `pg_db_role_setting` and refuses an add-on that parked anything, so parking a setting earns its author a failed load rather than a lever. And the leftover on an uninstalled add-on's role is inert, measured: a session default is read only by a session that **logs in** as the role, `SET ROLE` and `SET SESSION AUTHORIZATION` both leave the parameter at the cluster default, and nothing logs in as that role once the module is gone — while re-installing the add-on clears every scope before its pool opens a connection. An operator who wants it gone anyway types one `ALTER ROLE … RESET ALL` per scope, which [operations.md](operations.md) carries. It is not an escalation route: `NOLOGIN`, `CONNECTION LIMIT 0` and `temp_file_limit` are each refused to the role, and the two settings that would otherwise matter — the search path and the statement timeout — are overridden per transaction by the host. One thing the role *can* change about itself is its **password**, which Postgres allows every role and forbids to nobody: it is accepted through the write path, it outlives the process in `pg_authid`, and LNK-F-0280 is what it is worth. So the post-condition is what holds here too, and it reports a temporary relation whether the revoke took or not. **The role can also take one of this product's job advisory locks**, the keys being constants in a public repository and `pg_advisory_lock` being executable by any role — the product's own migrations use the same functions, so revoking that family is not available either. A session-level lock survives the rollback both storage paths perform, so the host releases every advisory lock a connection holds before that connection is reused, synchronously. The residual is one statement's worth: an add-on can hold a job's lock for up to the five-second statement timeout and retake it, which makes a job skip ticks — the same outcome as a follower losing an election, and the same class of nuisance as an add-on holding its four connections, which it may also do. |
| Add-on pages | **An add-on holding `routes.own_prefix` answers requests under `/addons/<name>/` on the dashboard host, and it does not write the HTML.** That is the load-bearing claim of this row and it is a property of the shape rather than of a filter: what a module returns is **text**, the content types it may name for itself are a closed pair — `text/plain` and `application/json`, neither of which a browser executes and both served with `nosniff` — and the default is that LinkCtrl wraps that text in its own page template, where it is escaped like every other value on every other page. There is no path by which a module's bytes reach a browser as markup, so `text/html` is **refused** at the moment the module writes it rather than sanitized afterwards, and the `Content-Security-Policy` is byte-identical to what every other page carries: a test holds the served header against a literal copy of the policy as it stood before add-ons could draw a page, because comparing it against the constant would agree with whatever the constant says. A module that answers with a script tag, an inline handler or an external reference puts the characters of one on the screen, and that is asserted against a real module which tries all three. The prefix is on the **application tree only** — the link host serves short links and nothing else, and the redirect tree's tripwire tests pass unmodified — and the prefix itself is in the reserved-word list, so no alias can shadow an add-on's pages. **Those pages are reachable without a session, deliberately**, because an add-on that authenticates somebody is answering a request from a person who has none yet; what it may learn about who *is* signed in is the separate `session.context` grant, whose record carries an account, an email, a display name, a workspace, an organization and a role, and carries no cookie, no token and no session identifier — so an add-on can draw a page for the person in front of it and cannot act as them anywhere else. **Three consequences are accepted rather than prevented, and they are stated because the directory is a trust boundary and this is part of what installing a module costs.** An add-on holding this grant can redirect a visitor **anywhere**, since sending somebody to an identity provider is the whole point of one; the host enforces that the accepted status beside a `Location` is `302` or unset and nothing else — the permanent forms and `307` alike — that a `Location` carrying a control character, a backslash or a scheme-relative `//host` is refused, that a bare 3xx with no location is refused, and that a redirect carries no body and no content type. Five rules, and this sentence said the host enforced two of them until 0.4.0; the count is now what the code does rather than what was true when it was written. An add-on's route runs code on an **unauthenticated** surface, which is why every request that reaches an installed add-on under `/addons/` is charged against the per-address login budget — the same `LINKCTRL_LOGIN_RATE_PER_MIN` the sign-in form spends, so alternating between the two gains an attacker nothing — and why the number of add-on invocations in flight is bounded at sixteen across the instance — and why each of those sixteen instances is bounded at **8 MiB** of guest memory, and eight more may be kept warm between invocations, a ceiling of **192 MiB**. **The sixteen are shared by every reason an add-on runs**, which since M66 is three: a page request, an inline redirect invocation and an out-of-band observation. They do not wait alike, and the difference is the redirect path's: a page request that cannot get a slot waits on the request's own deadline and is a 503 after it, an observation waits on the add-on deadline and is dropped after it, and an **inline redirect invocation never waits at all** — the redirect is served with the add-on skipped rather than queued behind one, because a visitor waiting for an add-on's *turn* is a visitor waiting for a resource this product owns. Both refusals are counted, on `linkctrl_rate_limited_total{limit="addon_inline"}` and `{limit="addon_observe"}`. Both bounds are needed and the second was missing until M64 was reopened: a request gets its own instance of the module, so sixteen unbounded instances priced an amount the module chose rather than an amount the host allowed, and one request that allocated took the process from 78 MB resident to 1604 MB. A module wanting more than its 8 MiB is stopped by the runtime — the request answers 502 and the instance goes on serving — a module whose memory section *demands* more than that as its minimum is refused at load, with the add-on named; and one that declares a larger *maximum* is not refused at all — the runtime replaces the declaration with its own limit, so the instance is held to 8 MiB whatever the module's toolchain wrote. **The rate limit covers every add-on the instance serves and not only the add-ons that can mint**, which is a decision with a cost rather than a free tightening: an add-on holding `session.mint` makes its route a credential endpoint by this product's own definition — a repeated assertion supersedes an account's outstanding second-factor prompt and writes an `audit_logs` row per attempt, neither of which any counter bounded — and a rule keyed on that grant would be a rule the next manifest could quietly move a route out of. So an add-on page carrying no credential pays against a limiter written for credential endpoints, and an operator whose visitors share one address may have to raise the number for add-on pages alone. **What is not charged is a path under the prefix that reaches no add-on**, which is the 404-probe limiter's rule applied here: the host answers 404 on shape, before the limiter and before a body is read, so an ordinary scanner walking `/addons/nosuch/wp-login.php` cannot spend the budget somebody needs to sign in — measured as a denial of `POST /login` before that shape test existed, and with `TRUSTED_PROXIES` unset a denial for every visitor at once. The two questions are answered by one function, so *what is an add-on route* cannot come to mean one thing to the limiter and another to the handler. **A request that would not fit the boundary is refused as the client's error, not the add-on's**: the host encodes the request record before it instantiates anything and answers **413** when that record exceeds the ABI's single-value bound of 64 KiB, so a large POST cannot reach a module that would then be unable to answer about it — which would have read in the log as the add-on failing. The ceiling on a body is therefore a function of its bytes rather than a number an operator can configure: the envelope sits beside it in the record, a body that is not UTF-8 is base64 first, and a control character costs six bytes inside the encoding. And a cookie an add-on sets is bounded the way one it reads is — the declared prefixes — with the path scoped to the add-on's own prefix and `Secure`, `HttpOnly` and `SameSite` applied by the host, so a module cannot opt out of them or widen its namespace to the origin. **How many cookies it sets is not a thing it decides either.** A browser's cookie store is small and evictable, and an add-on that could fill it could evict this product's own session cookie without ever naming it — measured in Chromium, where 180 cookies inside an add-on's own namespace signed the visitor out on every visit to its page. So the host does not write an add-on's cookies at all: it writes **one cookie of its own**, `linkctrl_addon_<name>`, carrying what the module set inside it, with a second `…_kept` when a lifetime outlives the browser being closed. A module still names its cookies and reads them back by name, and a bounded count is a property of the code rather than a threshold — it does not move when a module answers a thousand cookies, and it does not move when somebody visits a thousand times. What an add-on can still do is fill its own jar, which is 3 KiB: its oldest values are dropped and the log names it. **An add-on's configured settings are read from the environment** as `LINKCTRL_ADDON_<NAME>_<SETTING>`, only for the settings its manifest declares, and every value is held in the type that refuses to print itself — whatever the manifest called the setting — so an add-on's client secret cannot reach a log through a line about the add-on. The boot log says how many settings are configured and never which. |
| Add-on sign-in | **An add-on can assert that somebody authenticated; it cannot make a session, and it is never handed one.** `session.mint` is the highest-value grant in the vocabulary and it buys an *assertion*, not an authority: a module says *this external subject authenticated at this issuer*, and this product decides everything that follows. The host runs the same rules the password path runs, against the same columns — **the account must exist, be active and not be locked out**, and lockout applies even though no password was guessed, because a lockout is a statement about the account and an external provider is not a way around one. **The mint is a third caller of one function**, the same `mintSession` a password sign-in and a completed second factor reach, which is the mechanism that makes a second factor worth anything: there is exactly one place in this product a session token comes into existence, and this milestone did not add a second. **Nothing crosses back but an expiry and a flag.** The record an add-on receives carries when the session stops being one and whether a second factor is still owed; the token lives on a value that is not part of the ABI, is held in the same wrapper this product's own secrets are held in so it cannot print itself into a log, and reaches only the code that writes the cookie. There is no function in the published ABI that returns a token, a cookie or a session row, which is a property of the surface rather than a promise about the code — the credential blocklist that keeps `SessionContext` clean walks this record's field names too. **Account linking is explicit, and matching on an email address is absent by design.** An assertion resolves through one table keyed on `(add-on, issuer, subject)` and there is no statement in this product that resolves one by any other column — in particular not by the address the assertion carries, which is the classic account-takeover shape: a provider installed for one purpose could otherwise assert an administrator's address and be believed. An assertion for a subject nobody has connected mints nothing. **The two halves have opposite preconditions**, which is what stops either being used to do the other's job: minting is refused when the request already carries a session — a browser changing who it is signed in as on a module's word is the login-CSRF shape — and linking is refused when it does not, because a connection is only ever written for the person in front of the browser. An API key is not that person and is refused. **A second factor is not bypassed.** An account with TOTP enrolled meets its prompt after an assertion exactly as it does after a correct password; the assertion produces a pending login rather than a session, and the account's lockout counter is not cleared until the factor is met. An operator whose provider already performed one can say so per add-on, with the exact word `true` and nothing looser. **An add-on that signs people in is `required` whatever its manifest says**, which inverts the rule that the add-on declares its own failure class: the publisher cannot know whether this instance has another way in, and an instance that boots with sign-in silently missing is the worse failure. An operator overrides it deliberately, and a value that is neither class stops the instance rather than being guessed at. **What the host does not defend is stated.** The callback arrives as a GET redirect — `response_mode=form_post` is unsupported, because nothing is exempted from the cross-site refusal — and a GET is not what that refusal covers, so protecting a callback against a browser that did not mean to be there is OAuth's `state` parameter and is the add-on's. The host's guarantee is narrower and is absolute: a connection is only ever written for whoever is signed in, in their own browser, at that moment. **Every mint leaves a record**, naming which add-on and which issuer vouched and deliberately nothing about the external identity, so the erasure sweep has no new key to reach. **The record follows the session, not the assertion**, which matters for exactly the account it is most worth having for: an assertion against an account with TOTP produces a prompt rather than a session, so the provenance is carried through the prompt and the record is written when the second factor completes — a TOTP account would otherwise have a session nothing named an add-on as the minter of. Two nullable columns on the pending-login row carry it, and neither identifies a person. And deleting an account takes its connections with it, for the same reason it takes a password-reset token — a connection admits somebody with no password. |
| Add-on redirects | **An add-on may run inside the redirect path, and this product's published redirect measurement is core with no such add-on on it.** That is a boundary rather than a gap: latency on that path stops being ours the moment an operator installs a module there, and *availability does not*. The figure in [slo.md](slo.md) is measured with nothing inline, and the same page carries a run with a deliberately slow module beside it; an inline add-on's own latency is the add-on's, and `linkctrl_addon_redirect_duration_seconds{addon,class}` is a separate curve from `linkctrl_redirect_duration_seconds` precisely so an operator can tell whose it is. **Separate in both directions, which is what makes the boundary readable rather than merely stated**: the redirect histogram excludes the time an add-on held the request — including the whole deadline when the module was killed — so core's curve still describes core on an instance that installed one, and the two can disagree. What a visitor waited is `linkctrl_http_request_duration_seconds`, which includes everything. **The availability half is enforced and not promised.** An invocation is bounded twice — the module's own code by `LINKCTRL_ADDON_INLINE_DEADLINE` and this host's work starting it by `LINKCTRL_ADDON_INSTANTIATE_DEADLINE`, because instantiating is the host's cost on the host's machine and a module's package initialization can hang there — the runtime kills whichever bound is overrun, the redirect completes without it, and the kill is counted per module and per step on `linkctrl_addon_redirect_kills_total{addon,step}` — so a module that hangs is a graph an operator reads rather than an outage they diagnose. **The step is what makes the graph readable**: one bound over both meant an instance too slow to start modules was indistinguishable from add-ons choosing to do nothing, which is a silent loss of every inline guarantee an operator installed. Two further bounds sit under it. The extension point is **after the destination is decided and before the gates that spend anything**, so a veto cannot retire a one-time link on a visitor's way out — and a veto is a refusal visitors meet, so it is countable apart from every other one, on `linkctrl_redirects_total{outcome="vetoed"}`, which is zero on every instance with no inline add-on; the one write it does not undo is a split test's rotation, which advances by one step per veto and is documented rather than compensated. And an invocation draws from the same instance budget add-on pages draw from, **without waiting**: a redirect that cannot get a slot is served with the add-on skipped and counted on `linkctrl_rate_limited_total`, which is what keeps a slow module from turning into unbounded concurrency. A failed, killed or skipped module always means *allow, unchanged* — never a refusal — because a bug in an add-on must not be able to take somebody's links down. **Since 0.4.0 the instance an invocation runs in is reused, and the isolation that came free from destroying it is enforced for the guest's linear memory and not beyond it.** A fresh instance per redirect cost the visitor 11.05ms of startup on a path whose target is 20ms, so the host keeps instances in a pool; a pooled instance carries the guest's own memory, which would otherwise hand the next redirect whatever the last one left in it. What the host does about that is not a rule modules cooperate with: it copies the module's linear memory the moment package initialization ends and writes that copy back over the instance before anything else may have it, so a module that stored a destination in a package-level variable reads an empty one on the next redirect, and a test drives two redirects through one instance to say so. **What that reset does not cover is a mutable WebAssembly global or a table**, which the host cannot reach — wazero exposes globals only through a module's export section and the toolchain this product ships exports none — so a module built to keep its state there carries it from one visitor's invocation into the next, and across workspaces, because the pool is keyed by add-on and class and by nothing else. No add-on this product's own SDK can build reaches it: Go gives a program no way to place its own state in a global. A module installed from a URL is arbitrary bytes and is not so bounded. What it buys an attacker is aggregation across invocations and influence on another tenant's redirect, rather than access to anything the module could not already read per invocation — measured at M69.9, and the fix is a scheduled finding rather than a promise made here. An invocation that did not return cleanly is never reused — a killed instance has already been closed by the runtime and a trapped one is in a state no image describes — so a module being killed repeatedly degrades to an instance per invocation rather than filling the pool with dead entries. What reuse costs is memory held while nothing is running: `LINKCTRL_ADDON_POOL_SIZE` bounds how many instances are kept, `LINKCTRL_ADDON_POOL_TTL` how long one is kept for, and the ceiling above is what the two add to the sixteen in flight. **The image is a second copy and is not inside that ceiling**, which states guest memory: it is ordinary host memory under the same per-instance cap, held for as long as the instance is, so what an operator sizes for is that ceiling twice — see [deployment.md](deployment.md). Add-on **pages** are not pooled and still get an instance per request. Observation is the other class and is off the path entirely: it is fed from the click pipeline after the response and after the click is durable, its queue is bounded, and an observation that cannot be delivered is dropped and counted rather than allowed to delay anything. |
...
**Erasure does not reach an add-on's storage, and the ABI cannot tell one that a
subject was erased.** An add-on holding `session.context` is handed an email, a
display name and an account id; `storage.own_schema` lets it persist them, with no
cap and no expiry; and the erasure sweep enumerates this product's own tables. It
does take the one add-on-related row it owns — the `addon_identity_links` entry
that let a module sign that account in goes with the account, and has since M65 —
but an `addon_*` schema is not this product's table and the sweep knows nothing
about it. The same is true of the retention job,
whose scope is `click_events`, `visitors` and `audit_logs` and nothing else, and
of an add-on's log lines, which are the operator's log and are swept by nothing
here.

**This is wider than an unbuilt feature and is stated as such.** The ABI has no
account-lifecycle function of any kind, so cooperative erasure is not merely
unimplemented — it is not expressible on the published contract, and an add-on
could not act on the event if the host raised one. What that means for an
instance under a legal erasure obligation is that installing an add-on which
takes `session.context` and `storage.own_schema` puts personal data somewhere this
product's erasure does not reach, and answering for it is the operator's. Removing
the add-on and purging its schema is the blunt instrument that exists;
`docs/operations.md` has it.
...
(LNK-F-0177), the `"from"` array beside it
(LNK-F-0189), the address on the
invitations the erased account **redeemed**
(LNK-F-0181), and the inviter's own
notification (LNK-F-0188). The invitation
...
the next boot names them: the host enumerates `addon_*` schemas belonging to no
**installed** add-on and warns.

*Installed* rather than *loaded*, and the distinction is load-bearing rather than
pedantic. An add-on can be on disk and not running — its module fails to start,
or its manifest stops validating while the tables it created on an earlier good
boot survive — and until 0.4.0 the enumeration subtracted what had *loaded*, so
such an add-on's schema was reported as an orphan and the Add-on manager offered
it for purge behind a confirmation saying the add-on had been uninstalled. It had
not been. The host now records what it discovered on disk separately from what
started, and the boot warning, the manager's list and the purge confirmation all
read from that one answer. Nothing in this release deletes one, deliberately — a purge is an
...
**Add-on egress: an add-on may make outbound requests, and naming an origin is
what authorizes them.** An add-on that declares `network.fetch` may ask this
instance to make an HTTPS request on its behalf. **This is the first door in this
product through which a server-side request reaches an address somebody else
chose**, and it is disclosed here because that sentence is the whole of what is
new: everything else this process dials has a destination this repository fixed at
compile time or an operator configured for a purpose this product defines.

**What the add-on gets.** The manifest declares the *need* and can never declare
the destination — a setting marked `origin` carries no default and no options, a
manifest naming a host is refused at load, and there is no host, pattern or URL
anywhere in the schema. So an add-on's author cannot widen its own reach: no
version of an add-on, new or old, can name a destination in a file you do not
edit.
Beyond the origin, what the host does is not negotiable by either party: https
only; `GET` or a form-encoded `POST` and nothing wider; **no request header of the
add-on's choosing**, so a credential or a `Host` override cannot be smuggled onto
the wire; no response header back but the content type; every address the name
resolves to checked **at the moment of dialling** against an allowlist — an address
is dialled only if it is in globally-routable unicast space, so loopback,
link-local (the cloud metadata service above all), unique-local, the private ranges
and any range nobody has thought about are refused however the name arrived at
them, and the log line names the rule under `address_rule=`. **A handful of
addresses are refused from *inside* routable space too**, and today that is
Azure's WireServer at `168.63.129.16` — a metadata endpoint that is not a
destination and does not sit in any private range, carved out by name (D383). So
being routable is necessary and not sufficient, and an operator whose origin is
refused with a rule they did not expect should read the carve-outs beside the
policy rather than assume a bug; no redirect followed off the origin it started
on; a response size cap **on the headers as well as the body**, because a response
this host holds in memory to hand across the add-on boundary is memory whichever
half of it is large; a request timeout; and no connection kept alive across
invocations. A fetch is callable from an add-on's own page handler and from
nowhere else — both redirect classes are refused, so nothing here touches the
redirect path.

**Upgrading an add-on is when to re-read its settings.** The paragraph above is
about the manifest, and it is exact: a manifest cannot name a destination. What a
new version *can* do is mark a setting you already filled in as one that names an
origin. Suppose v1 declares `homepage` as ordinary text and you type your own URL
into it; v2 declares `network.fetch` and marks that same setting `origin`. The
value is still yours and this server now dials it. Nothing was escalated —
installing a version costs `addons.manage`, which is the highest grant here and
non-delegable, and whoever holds it could have typed an origin themselves — but
you would be seeing a new permission without seeing that an existing value became
a destination, and this product's rule is that only you decide where an add-on
reaches. **So when an upgrade adds `network.fetch`, open the add-on's page and
read every setting marked as naming an origin before you trust the upgrade**, the
way you would read the permissions. The manager marks them; the values are
yours to clear.

**What you are trusting when you name one.** You are authorizing this server to
connect to that origin, repeatedly, with a request body the add-on composed, and
to hand the response back to code you did not write. The origin's operator learns
that your instance is talking to them and can see the source address it comes
from, and the request identifies itself: the `User-Agent` is
`LinkCtrl/<version> (+add-on <name>)`, so the origin also learns which build you
run and which add-on is calling it. That is deliberate — an origin's operator
receiving unexplained traffic should be able to tell what is generating it — and
it is the same disclosure this document makes above about the update check's own
`User-Agent`, stated here rather than left to be discovered by reading a log. If your instance sits somewhere with reachable internal services, the
address refusals above are what stands between an add-on and them — and as this
document says elsewhere about the destination validator, a deployment that cannot
afford to rely on that should put the egress control in the network rather than
here.

**An add-on holding this and `storage.own_schema` can send its own data
anywhere you configured.** That is not a defect; it is what you authorized, and it
is the first time two grants in this vocabulary compose into something worth
stating on its own line. It is not the only such pair, and this sentence read as
if it were until M69.9: `config.read` with `network.fetch` sends the *operator's
own* configured secrets to a named origin, which is what a token endpoint reached
with `client_secret_post` requires and is the shipped OIDC add-on's ordinary
work. That composition is set out under an add-on's settings further down — the
difference is that this one moves data the add-on **gathered**, which you never
handed it. The two are separate tokens precisely so that an operator
can see both before installing.

**It is a different door from the destination validator, with different rules,
and neither is a way around the other.** The validator (M30) judges where a
*visitor's browser* is sent, refuses private ranges and the metadata service at a
single call site, and its subject is a URL an end user typed. This judges where
*this server* connects, and its subject is a URL an add-on composed inside an
origin an operator authorized. They share the refusal of private space and nothing
else: the validator has no allowlist because a link may point anywhere public, and
this has one because an add-on may not. An add-on with an inline redirect grant
cannot reach either door — it may alter a destination's query string and nothing
about its host, and it may not fetch at all.

**Nothing is on by default and nothing is inferred.** An instance running no
add-on that declared the permission opens no such connection; an add-on that
declared it and has no origin configured reaches nothing and says so with the
`unconfigured` outcome. What happens is visible without reading logs:
`linkctrl_addon_fetch_total{addon,outcome}` and
`linkctrl_addon_fetch_duration_seconds{addon}` carry every attempt and every
refusal by add-on, and the Add-on manager renders both.
...
*This paragraph was wrong in both directions until 0.2.0* (LNK-F-0017),
...
- **Mounting the add-ons directory writable makes the server a writer to it, and
  that is the decision the install API asks you to make.** `POST /api/v1/addons`
  uploads a module and starts it without a restart. With the directory mounted
  `:ro` it answers `503` and the boundary above is unchanged — the only writer is
  somebody with filesystem access. With it writable the process can put code into
  the directory it executes from, on the word of whoever holds `addons.manage`.

  **Three things bound that, and they are what make the trade defensible rather
  than merely available.** The permission is an instance-level scope held by the
  account that administers the box, so it is the same set of people who could
  already reach the filesystem; it is **not delegable to an API key**, because a
  key that could install an add-on would carry whatever that add-on's manifest
  declares — including `session.mint`, which decides who is signed in — past
  every scope the key was issued with; and the bytes are settled by a digest
  before anything is written, whichever way they arrived. Every install and every
  removal is in the instance-wide audit log with the module's name, version and
  digest, and a URL install writes one more line naming the origin it came from — **in the server log, not the audit log**, so an operator reconstructing where a module came from reads the two together rather than the audit record alone.

  **This third bullet used to say that nothing is ever fetched from a URL**, and
  since M68.6 that is no longer true: an install may name a URL for this instance
  to fetch a bundle from. What the operator is being asked to accept is stated
  rather than folded away — **you are trusting the URL's host to serve the bytes
  your digest names, and the digest is what makes that a bounded trust rather
  than an unbounded one.** Bounded, because a compromised host that serves
  anything else is refused before it is parsed; and only bounded, because a
  digest copied from the same page as the URL is not independent of it. Where a
  fetch may reach is not the operator's to widen: `https` only, addresses checked
  after resolution against the globally-routable-only policy, no cross-origin
  redirect. Nor is what a bundle may contain: three container formats, one member
  rule of two plain files at depth zero, and a second size bound for anything
  compressed. The uploaded-content row above is where the mechanism is.

  **What it does not bound is what the manifest asks for.** The upload runs the
  same checksum verification and the same manifest parsing a boot-time load runs,
  and neither is a judgement about whether the code should be here. The bullet
  below is still the one that matters, and it now applies at a moment when
  nobody is at a shell.
- **Removing an add-on leaves its data.** Removal unloads the module, releases
  what it held and takes its files out of the directory; the Postgres schema it
  owned stays, and the removal's answer names it. **So does the login role
  `addon_<name>`, with its password**, which is the half worth saying out loud
  because it is a live credential rather than idle data: no code path in this
  product drops one, and `docs/operations.md`'s purge block is where the
  `DROP ROLE` lives for an operator who wants it gone. That is deliberate — data
  an operator has not asked to lose is not deleted by a lifecycle operation, and
  a role dropped out from under a schema somebody may still want to read is
  worse than one left standing — and it means an instance accumulates `addon_*`
  schemas nothing is reading, each with a role that can still reach it, until
  somebody purges them.

  **The sharpest limb of this is the one an operator is least likely to
  predict**, and it is worth stating separately because M67 makes
  remove-then-install the documented way to replace an add-on: **two host tables
  are keyed on an add-on's name rather than on its module**, and a replacement
  inherits both. `addon_identity_links` is unique on `(addon, issuer, subject)`,
  removal deletes no row from it, and the orphaned schema is reattached by name
  too — so a *different* module installed under a name a previous one used
  inherits every account mapping that one wrote, and can mint a session against
  those mappings on its first assertion. `addon_settings` (0.4.0) is the second
  and behaves the same way: the values an operator typed into the manager,
  **including a stored `secret`**, are read back by whatever holds the name next
  through `config_get`. This is not an escalation — the actor is the instance
  principal, who could always have swapped the module by hand, and the whole
  point of `addons.manage` being non-delegable is that this is the level of
  trust it carries. It is stated because *by name* is an inheritance rule
  somebody replacing a module will not think about, and because **nothing in this
  product clears either of them**. A purge is `DROP SCHEMA … CASCADE` and nothing
  else, so what a purge leaves is four things: the `addon_<name>` login role and
  its password, any large objects that role owns, the identity links written
  under the name, and the settings saved under it — the last two of which the
  manager counts for you at the point of decision and deletes neither. An earlier
  draft of this sentence said the purge that clears the schema clears the mappings
  too; it does not, and the counts exist because it does not. **The settings are
  the one with no route out at all**: a settings write is refused for a name that
  is not loaded, so once an add-on is removed its stored values are unreachable
  from this product entirely, and `DELETE FROM addon_settings WHERE addon = …` at
  a `psql` prompt is the only way to be rid of them. The purge's
  confirmation says all four, because a delete that quietly leaves things is
  worse than one that does not. They are named rather than merely present: the boot log
  warns about every schema no loaded module owns. Their size gauges stop —
  nothing measures a schema no add-on owns, so the series is dropped rather than
  left frozen at its last reading.
- **Read the `permissions` in a manifest before you install it.** They are the
  ...
  your database. **Since 0.4.0 the Add-on manager lists them** — on
  `/instance/addons`, per module, as the set the manifest *declares*, with any this
  build grants to nobody struck through so that what is withheld is visible rather
  than missing — and they are also in `addon.json`, in the boot log, and on
...
- **Configuring an add-on is a write into what it does, and the manager is where
  it happens.** Since 0.4.0 an add-on's declared settings are editable on
  `/instance/addons/<name>` behind `addons.manage`, and a saved value changes what
  the module does on its next invocation. Four things bound it and each is stated
  because none is obvious. Only settings the **manifest declares** are writable, so
  the page cannot invent a key a module was not written to read. A `secret` is
  never echoed back — the field says *set* or *not set*, and the value leaves the
  database only for `config_get`. **That withholding rests on the row and not on
  the manifest in hand**: the stored value records what it was written as, so a
  replacement add-on installed under the same name cannot have its predecessor's
  credential rendered back into the form, or returned by the API, by re-declaring
  the setting as `text` — which matters because replacing an add-on is a removal
  and an install. **It bounds the page and not the module, and the difference is
  the thing to read twice.** Stored settings are keyed on the add-on's *name*, the
  way the identity mappings are, so that replacement's own code still reads the
  credential through `config_get`. Nothing about installing a module under a name
  somebody else's module used is safe, and this is the second reason on this page
  why. What the withholding buys is that a **person** cannot read the value off a
  page they can reach; what it does not buy is anything against the code they
  chose to install. The alternative — withholding it from `config_get` as well —
  was declined deliberately: the page would then say *set* while the module read
  nothing, which is a misconfiguration with no surface naming it. It is stored **in the clear**, exactly as
  `LINKCTRL_ADDON_<NAME>_<SETTING>` sits in the clear in the process environment;
  encrypting it would need a key living beside the database in the same
  environment. A setting your **environment** answers is refused rather than
  overridden, so a value pinned in the deployment cannot be changed from a web
...
- **An add-on's words can reach your sign-in page, and only because you said so.**
  ...
  for you. **What a new version can still change is the words**: the consent is
  stored against the add-on's *name*, this product has no upgrade-in-place, and
  so the replacement installed under that name inherits the toggle and draws its
  own label. Nothing records the label you agreed to and no page shows it to you
  — turning the toggle on is consent to a string you have not read. Reaching it
  costs `addons.manage`, which is the permission that could set the toggle
  anyway, so nothing is escalated; M69.9 filed it.
...
- **An authentication add-on moves part of who-may-sign-in to somebody else, and
  ...
  refuses to proceed without — **that release's own `SHA256SUMS` sits on the
  page the link was on and therefore authenticates nothing** — whoever can change
  the one can change the other. `v0.1.0` is the first release. What does answer
  the question is the build provenance it publishes, over the **bundle** as well
  as the module, naming the workflow, the tag and the commit that produced it,
  which `gh attestation verify linkctrl-oidc-0.1.0.tar.gz --repo
  DevOfPie/LinkCtrl-OIDC` checks — signed by GitHub's workflow identity and
  publicly logged, so it is the one channel that is not the page you were served — **that question is yours to ask before
  ...
  Every mint is recorded as `session.minted_by_addon`, naming the add-on and the
  issuer and carrying neither the external subject nor the address the assertion
  asserted. **Read it in the audit log of the organization the session resolved
  to, not the instance-wide one** — the record is scoped to that tenant, so
  `audit.read` there is what reaches it and `audit.read.instance` does not. The
  one exception is an account with a second factor: the record that the assertion
  arrived and a second factor was demanded *is* instance-wide, because no
  workspace has resolved yet, so for such an account the two halves of one
  sign-in land in two different logs. This is the surface as built and it was
  nobody's decision — no `D` number chose it — which is why it is written down
  here rather than corrected quietly; M69.9 filed it.
