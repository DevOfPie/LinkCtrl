...
them, and the log line names the rule under `address_rule=`; no redirect followed off the origin it started
...
- **Mounting the add-ons directory writable makes the server a writer to it, and
  ...
  **Three things bound that, and they are what make the trade defensible rather
  ...
  every scope the key was issued with; and nothing is ever fetched from a URL, so
  the bytes are the ones the caller sent rather than the ones some address
  answered with. Every install and every removal is in the instance-wide audit
  log with the module's name, version and digest.
