...
| Domain ownership | `domains.write` is an **ownership** check rather than a flat permission: a workspace admin registers, renames and removes their own workspace's hostnames and gets `403` — not `404` — on another workspace's, because being told a hostname is not yours is the true answer and does not invite guessing at ids. Another workspace's hostname is **absent from the list**, not present and unmanageable; the registration is the only thing there is to disclose. **A hostname belongs to exactly one workspace**, enforced instance-wide by `domains_hostname_key` on `lower(hostname)`: a hostname is one alias namespace, so sharing one would mean two workspaces racing for the same alias with nothing to arbitrate. The conflict message names the hostname and never its owner, so registration cannot be used to ask what a neighbour has registered. The instance's **default domain** keeps the guard it has had since it was minted — `domains.write`, granted to the owner and admin roles alone — and the collection additionally refuses to rename or delete it whoever asks. **Nothing is served on a registered hostname**: it is stored unverified, no `Host` header is resolved against the table, and an unrecognized host gets the operational `404` exactly as before. Nothing checks that a registrant controls the name, deliberately — that is DNS's question and verification's answer, and a syntax check rejecting a few recognizable names would read as protection while proving nothing. Registering, renaming and removing are audited. |
