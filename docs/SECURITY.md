...
**Erasure does not reach an address held in an audit record's `metadata`.** The
sweep described above scrubs the *actor* snapshot — `audit_logs.actor_label`, and
both label columns in `destination_disputes` — and nothing else. Several actions
carry an address in their metadata as the **subject** of somebody else's act:
`invitation.created` and `invitation.redeemed` name the invited address,
`member.added`, `member.removed` and `member.role_changed` name the member's. An
erased account's own redemption record therefore still holds their address, and
so does the record of the administrator who invited them.

The reach is bounded and it is not a hole in the rest: nothing here is reachable
without `audit.read`, which is not delegable to an API key, and the records
themselves are governed by `AUDIT_RETENTION_DAYS`. Closing it is a design
question rather than an oversight — the address sits inside a record of what
somebody *else* did, so removing it edits their audit entry — and it is recorded
as [F177](build-notes/deferred-findings.md) rather than decided inside the
