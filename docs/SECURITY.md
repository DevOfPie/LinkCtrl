...
- **Read the `permissions` in a manifest before you install it.** They are the
  ...
  clock: there is no load-time deadline (F267), so a module that never returns hangs
  the boot whatever it declared. Migrations are a second way to reach that same
  hang and not a new one — they run in the same before-listener window, under a
  Postgres session lock that waits up to five minutes for another replica.
