...
- A value the host returns crosses into a buffer **the guest owns**, passed as a
  **(pointer, capacity)** pair, and the return value is the size the value
  ...
  something makes sure the retry is not a second attempt**, and there are two of
  them here — the two with both an out parameter and a side effect — answering it
  two different ways.

  `session_mint` **checks the buffer before it mints**, against the record at its
  widest rather than against the answer it is about to produce, because that
  answer does not exist yet. So *nothing was written* also means nothing happened.

  `network_fetch` cannot do that — its widest answer is the operator's response
  size cap, and demanding that much buffer before any request may leave would
  price every add-on for the largest document any add-on might read — so it
  **holds what came back and answers the retry from it**. The request goes out
  once. The hold lasts for the one retry and no longer: it is dropped as soon as
  it is delivered and dropped by a fetch of anything else, so a module that
  deliberately fetches the same URL twice gets two requests. None of this is
  visible to a guest, and the reason it is written down is that the alternative
  was invisible too — an authorization code exchanged twice, the second time
  refused.
...
The ABI is **0.1.5**, generation **1**, and this host loads generation 1 or newer.
