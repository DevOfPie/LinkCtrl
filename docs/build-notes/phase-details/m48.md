...
- **Its contents stay reachable without JavaScript.** The panel is a route that
  ...
  shell is already inconsistent about it — `pages/links.html:122` has a
  `<noscript>` submit, and the workspace switcher deliberately does not
  (D103/F21, `partials/nav.html:9-36`) — and a milestone that adds a third
...
The demo already seeds four notification kinds — `domain.failing`,
`automation.fired`, `dispute.filed`, `dispute.decided` — so the click-through
