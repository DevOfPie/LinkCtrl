# M48 — On-demand panels, and what stops being buried

**Depends on:** [M47](m47.md). The edge is hard: the panel pattern is applied to
the page M47 restructured, and defining it against the old page would mean
defining it twice.
...
- **The download control gets an icon and keeps its text.** The note was
  ...
  *(**Superseded 2026-08-14 at [M50.7](m50.7.md) — by the owner, not by a
  ...
  [F224](../deferred-findings.md)(d) in the owner's own words: *"the download
  ...
  affected. Marked rather than reopened: M48 built what it claimed, and a later
...
- The retrieval path is measured against the blind task that produced it: task 1
  ...
  than asserting an improvement. [M51.9](m51.9.md) re-runs the task.
...
[F212](../deferred-findings.md#closed): *"everything from it can just go on
the QR tab, clicking the QR code should open the tab."* M48 built the panel
because the stack buried the section; M47's tabs un-buried it, and one
click now reaches a tab rendering what the popup duplicates. Work below
carries M48's number.

Reviewed by [M57.9](m57.9.md) — M48 sits inside its range.
...
- **The page-level QR popup is retired**: the `link_qr_panel` invocation
  M47 kept at page level goes, and whatever the popup showed that the QR
...
- **M48's other panels are untouched**, and any of its claims this
