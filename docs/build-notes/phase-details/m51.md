...
- A `password_resets` table in migration `03700`, modelled on
...
- **An account whose `status` is not `active`** (`internal/auth/service.go:377`).
...
- The body carries a live token, so it depends on `03200_mail_body_scrub.sql:45`
