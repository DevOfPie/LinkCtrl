...
- **Make default becomes an icon on *every* row, filled on the default and
  ...
  *Enforced by:* a template test asserting the list renders exactly one filled
  ...
  the flag, and that each carries an accessible name; plus a browser case
  clicking an empty icon and asserting the fill moved.
...
- **Anchoring is the part that is new.** The existing pattern uses a single
  ...
  anchor names**, generated per row from the code's slug, or every menu
  anchors to the same element. Where `anchor-name` is unsupported the existing
  fallback is viewport-pinned, which is meaningless for a control halfway down
  a list, so the fallback for these is the row itself.
