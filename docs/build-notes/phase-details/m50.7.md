...
**Depends on:** [M49](m49.md) *(**third** reopening, landed `fc9aeef` — the size
...
deleted)*, [M50](m50.md) *(reopening, landed `2d4f424` — the codes list this
milestone re-lays-out is the list that reopening makes fully removable, and it
is what put a **Make default** control on every row)*, [M50.6](m50.6.md)
...
*(Re-cited 2026-08-13 at step 1, which is [W39](../workflow-changes.md#made)'s
...
went on to delete a paragraph that **was** M49's shipped claim and
[F213](../deferred-findings.md#closed)(a)'s closure. It is true again rather
than merely restated: the owner routed that deletion into M49's third reopening
...
- **A row selects on its whole area**, not only on the name. Today only the
  ...
  *Enforced by:* a template test asserting the row's selecting element spans
  ...
  [D24](../../../Plan.md#phase-2-decisions-taken-after-the-plan-was-finalised) — *"a top-layer element ignores its
  ancestor's containing block, so positioning is verified in a browser rather
  than asserted from markup"* — and [m47.5.md:91-94](m47.5.md)'s kept spec both
  set.
...
- **Make default becomes an icon on *every* row, filled on the default and
  ...
  - **The set is exclusive and it is drawn that way.** One filled icon per
    ...
    ([D183](../decisions.md)). That is a radio group's semantics, so each icon
    carries an accessible name naming *its* code and the pressed state is
    exposed rather than left to the fill.
    *(Amended 2026-08-14 at [M50.8](m50.8.md)'s step 3.4, fact-level and
  ...
  - **It renders for a reader who cannot change it**, as the static pair with
    ...
    [D190](../decisions.md), after the first attempt put the whole set inside
  ...
  *Enforced by:* a template test asserting the list renders exactly one filled
  ...
  [M50.8](m50.8.md): the name is asserted of the button, the icon being
...
2026-08-13 was to keep M50.7 as **one** milestone with that cost written into
...
- It is **not an ARIA menu**, and this file does not call it one.
  [m26.5.md:89](m26.5.md) settled that for the header's popovers and the same
...
- **Anchoring is the part that is new.** The existing pattern uses a single
  ...
  `nav.html:173` — and [m46.6.md:230-235](m46.6.md) calls that the
  ...
  *The **first half was wrong on a fact**: the disjunction has a third limb the
  ...
  no stylesheet. Corrected, logged, carried on — [D189](../decisions.md).*
  ...
  *The **second half was an assertion**, and it went to the owner rather than
  ...
  real choice between three. Owner-answered 2026-08-14, [D190](../decisions.md):
...
- **The server action stays**, owner-set 2026-08-13 over deleting it with the
  ...
  *control* is not removing a
  ...
  [README.md](README.md#what-every-milestone-inherits) — does not run
...
- **The encoded URL under the picture goes.** Owner-set.
  *(The paragraph under the preview is **already gone** — M49's third reopening
...
*(**Amended 2026-08-14 at [M50.8](m50.8.md)'s step 3.4, fact-level: the bound is
750.** That milestone deleted two more paragraphs on the owner's instruction and
re-measured by the method above — four paragraphs at 100, 153, 191 and 271, so
**715**, rounded up to the next fifty. The instruction one paragraph up is what
obliged the move and it was followed, in the file it names. Nothing about the
method, the 40-character floor or the owner's choice of 900 over 1200 and 1500
is withdrawn; 900 was the right bound for the tab M50.7 left behind.
**The 911-against-900 arithmetic above is spent** — it was computed against the
last-code and second-code paragraphs, both of which M50.8 deletes, so the
required sentence under the floor now sits against 715 and the headroom is 35
rather than ~23. See decisions.md, D193.)*

**The logo paragraph is inside this bound and its controls are still not
restructured.** That is not a contradiction and the plan review was right to
ask: M50.6's second reopening rewrote its level sentence four hours before this
...
`partials/icons.html`, which is where [m47.5.md:79-81](m47.5.md) put the first
...
*(Amended 2026-08-14 at [M50.8](m50.8.md), fact-level: the shape now has two
...
what [m47.5.md:79-81](m47.5.md) established and a milestone re-laying-out a
...
[M49](m49.md)'s second reopening deleted, so it goes with it; the contrast half
