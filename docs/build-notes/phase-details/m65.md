# M65 — The authentication hook: a session minted on an add-on's word

**Depends on:** [M61](m61.md), [M62](m62.md); [M64](m64.md) *(hard — an OIDC
flow needs the callback route M64 built)*.
...
- **The host mints; the add-on asserts.** `mintSession`
  ...
  (`session.minted_by = addon:<name>`). The add-on never sees a token, a
  cookie, or the session row; asserted by the ABI's surface — no function
  returns them.
- **The grant is its own vocabulary entry** (M62), held by nothing by default,
...
- **What this deliberately does not decide: provisioning.** Whether an unknown
  ...
  that wants it, recorded in [phase-4-candidates.md](../phase-4-candidates.md).
- **MFA composes; it is not bypassed.** An account with TOTP enrolled (M53)
...
- **Sabotage-verified end to end** with a test module asserting a fake
  identity: wrong subject, unlinked subject, locked account, disabled add-on —
  each refused at the host boundary, each a test that has been made to fail.
...
- This is the milestone where a host bug is a CVE. The mitigation is the
  ...
  password path's, and the linking table as the only bridge. The M69 OIDC
  build and the M69.9 review both attack it.
- Session provenance in the audit trail must not break Phase 3's erasure
  sweep assumptions (M52's tombstoning reads `audit_logs` shapes); the new
