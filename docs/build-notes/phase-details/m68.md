...
- **Install and remove are driven from the page through M67's API** — same
  ...
  *Remove selected (n)*, opens **one confirmation for one or many**: a
  purge-data choice per selected module (its M63 schema), default off, and a
...
- **The detail page renders and saves the add-on's declared settings**
  ...
  [phase-4-candidates.md](../phase-4-candidates.md#the-managers-layout-chosen-from-wireframes-amended-confirmed)):
  settings declared in the manifest (M60) with a type each — text, secret,
  ...
  and readable by the module through M61's ABI. Secrets get the `Secret`
  treatment and are never echoed back into the form.
  *(**Amended 2026-08-29 under [M69.5](m69.5.md), a fact.** What M68 built is
  ...
  **declared** settings", because since M69.5 this page also renders one setting
  ...
  keys M64 reads stay the operator's, and nothing on this page edits them. See
  decisions.md, D401.)*
...
- **Deliberately not done:** add-on marketplace/discovery anything; editing
  the operator's *environment* config from the page — the env keys M64 reads
