# M69.5 — Somebody can start the sign-in an add-on made possible

**Depends on:** [M65](m65.md) *(the grant that makes an add-on an authenticator)*,
[M64](m64.md) *(the route the control points at)*, [M68](m68.md) *(the manager,
where an operator sees what an add-on declared)*,
[M69](m69.md) *(hard — the finding is M69's and the add-on that proves this is
M69's product)*.
**Discharges:** [F345](../deferred-findings.md#closed) — an add-on that can mint a
session cannot put a sign-in control anywhere a person will see it.
Owner-answered scope, 2026-08-27.

M65 gave an add-on the power to authenticate somebody and M64 gave it a page.
...
- **The sign-in page offers what an installed add-on declared, and nothing an
  ...
  the host's to compose**: it joins the add-on's own M64 route prefix to the
...
- **The manifest format's three obligations are met, because a new key is what
  ...
  rule exists to catch. **`schema_version` stays `1`**, owner-answered on
  the precedent M61's cookie-namespace entry records: no LinkCtrl release has
...
- **Nothing appears on a stock instance.** No add-ons directory, or no installed
  ...
  renders today — asserted by a template test, because this is the one page every
  visitor with an account meets and the cost of getting it wrong is the whole
...
- **The operator decides whether it appears at all.** Declaring the label is the
  ...
  an add-on-declared setting riding M64's types and M68's page, the rule is
  already discharged by M68 and this milestone says so; if it is a new host
  operation, it lands in `api/openapi.yaml` under `addons.manage` and is replayed
  by the contract test, the way M67, M68 and M68.6 each did. Decide which and
  write it down — the rule is not optional and neither is saying which limb
  applies.
  The operator's consent follows [D364](../decisions.md)'s shape — the manifest
...
- **The OIDC add-on gains the two fields and its README stops telling an
  ...
  *stated here* for whoever lands it — the same shape as
  [M70](m70.md)'s obligation to bump that repository's `go.mod`.
...
- **A broken add-on must not break the front door.** If a module holding
  ...
  M67 reached** for its name-collision check, which runs over `claimants` and
  ...
  reader who knows M67 will otherwise think this one has it backwards.
