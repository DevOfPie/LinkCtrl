# M57 — High availability: measured, and still one container

**Depends on:** [M56](m56.md). The edge is hard — this milestone measures the
contract M56 writes, and a measurement of an unstated promise measures nothing.
...
M56 says what the product promises. This milestone proves it, and proves the
...
- A measured rolling replacement of every replica while traffic runs, using the
  existing k6 harness from [docs/slo.md](../../slo.md) against the composed
...
This is one of the phase's **three** k6 runs. The others are [M50](m50.md)'s,
which parses a second query parameter on the redirect path, and
[M57.9](m57.9.md)'s re-measurement on the final build. It is worth naming that
[phase-3-candidates.md](../phase-3-candidates.md) excluded work area D partly
...
three. *(Corrected 2026-08-07, from review: this said two and so did M50, while
M57.9 has always required a re-measurement on the final build. The count was
...
`cmd/linkctrl/jobs.go:117-127` states it plainly — during a rolling deploy the
...
- **Or measures it**: how long the window is in a real rolling deploy, and what
  each of the seven families actually does when it opens. Then the contract in
  M56 gains a stated bound instead of a stated possibility.
...
- It asserts **no new required dependency** was introduced by M56 or M57: the
...
M56 wrote, the contract changes — not the measurement. Per
[workflow.md](../workflow.md), a defect that makes a shipped milestone's claim
false reopens that milestone, so M56 comes back rather than M57 shipping beside
...
phase's shape, because a failed rolling-deploy measurement reopens M56 and
...
all seven job families are safe under two leaders is a proof about seven
different pieces of code, and the honest answer may be *six are and one is not*.
