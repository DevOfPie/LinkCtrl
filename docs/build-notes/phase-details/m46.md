# M46 — The shell, the navigation, and the links list
...
- The M25 decision that produced that guard is **honoured, not reversed**: *"a
...
- The `<noscript>` submit (`pages/links.html:122`) keeps working. A filter
  control that only opens with JavaScript would make the list unusable without
  it, and this product's dashboard degrades rather than requires.
...
- At 360px the shell does not scroll horizontally. Today it does: *"page needs
  ...
  switcher."*
- Enforced by a template scan in [M24.5](m24.5.md)'s idiom rather than by
  looking: any element that can exceed the viewport — tables first — is inside a
  container that scrolls on its own, asserted across every page template so a
  page added later cannot reintroduce it.
...
The same falsifiable claim [M47](m47.md) makes, for the same reason: **every
