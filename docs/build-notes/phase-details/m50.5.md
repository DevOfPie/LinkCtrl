# M50.5 — The first file this product accepts

**Depends on:** [M50](m50.md), and the edge is **hard**. The image this
milestone stores belongs to a QR code, and M50 is what makes a link have more
...
a QR code is [M50.6](m50.6.md), and the two were one milestone until a review
...
- Authorization is `links.update`, the permission that already governs QR style
  (D75, `internal/httpx/api_qr.go:18-20`). **No new permission**, so there is no
...
implies** — the standard [M49](m49.md) set for its own output cap. A test feeds
...
milestone picks one and writes down why**, against three constraints that
already exist: `docker-compose.yml` mounts only `pgdata`, so a filesystem path
needs a volume that does not exist and that `make demo-update` must not lose;
[M57](m57.md) plans a conformance test asserting a single container with **no
new required dependency**, which an object store would be; and a column puts
binary in the row and in every backup.

The question is filed in
[upcoming-decisions.md](../upcoming-decisions.md) rather than left in this file,
because it is exactly what that file is for and the loop will stand still on it.
...
- **Deleting the code** ([M50](m50.md) adds that operation), the link, the
...
[M52](m52.md) gains a limb for uploaded content, and **its reach is narrower
than an earlier draft claimed**. Logos hang off `qr_codes` → `links` →
workspaces and organizations, never off `users`; and M52 refuses to delete an
...
both above. M52's section is written to say that rather than the reverse.
...
are standard library — `image/png` arrives with [M49](m49.md), `image/jpeg`
...
repository. What the demo *shows* is [M50.6](m50.6.md)'s; what it proves here is
...
test: an object store fails M57's conformance assertion before M57 is written.
