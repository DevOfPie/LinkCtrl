...
- **No JavaScript.** `ui` is stdlib-only, the CSP forbids inline handlers, and
  ...
  progressive-enhancement idiom as [M38](m38.md)'s tree and M24.5's theme control.
...
`<details>` menu is keyboard-operable but is not an ARIA menu, and screen-reader
