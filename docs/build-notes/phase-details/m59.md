...
[F255](../deferred-findings.md#open) is *nothing asks whether CI is green*, and
the phase should not run without that gate.
**Discharges:** [F248](../deferred-findings.md#open),
[F253](../deferred-findings.md#open), [F254](../deferred-findings.md#open),
[F255](../deferred-findings.md#open) — the four process-debt rows the owner
...
- **F255 — a CI gate exists, and it joins an enforced chain rather than a
  ...
  F255's evidence (`Makefile:165-168`'s unpinned-shellcheck argument,
  suppressions at `scripts/rolling-deploy.sh:102` and
  `scripts/single-instance-check.sh:66`) is the case it must catch: local green,
...
- **F248 — `check-links` learns table membership.** For links whose fragment is
  ...
  which anchors are row tables, as F248's fix note says. The 17 wrong links in
  maintained files (`Plan.md` ×3 and the **ten** milestone files F248 enumerates — its own cell says nine over a ten-file list, counted here rather than trusted)
  are corrected in the same diff; the 22 in `decisions.md` are left, that file
  being append-only — each was true when written.
- **F248 — sabotage-proof.** The new check is shown to fail on one of the 17
