# M46.5 — A browser an agent can drive, and a test that outlives it

**Numbered below [M57.9](m57.9.md), which reopens with it**, per
[W43](../workflow-changes.md#made). **Numbered before [M47](m47.md)** because M47
...
what makes it admissible at all: **[D25](../../../Plan.md#phase-2-decisions)** —
...
2026-08-11**, from research commissioned after M47 reopened.
...
1.62.1 in [`tools/render-verify/`](../../../tools/render-verify/README.md) with
engines installed. **Three harnesses are explicitly recorded as not committed** — M46
(`decisions.md:20138`), M47 (`:20392`) and **M48** (`:20930`) — each on the same
...
Two further ad-hoc runs out of the same `node_modules` are recorded at M51.9's
and M57.9's acceptances (`:22196`, `:23898`) without a discard note. So: **three
discarded deliberately, five uses in total.** *(An earlier draft said five
discards and named M53, which appears nowhere in that record; it is M48.)* This
...
- **htmx's CSP violation is fixed here, not in M47.** htmx 2.0.9 injects a
...
- It is here because **this milestone's kept spec names a clean console as one of
  ...
  impossible. The owner approved the fix into M47; the plan review found that M47
...
- **`X.9` reviews carry a browser check**, stated in
  [phase-loop.md](../phase-loop.md)'s review section — owner-set 2026-08-11, over
