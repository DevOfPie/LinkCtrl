# M54 — An API key belongs to an account, not to one organization

**Depends on:** [M52](m52.md), and the edge is **hard but narrow**.
...
because nothing wrote that column; M52 gives it its first writer. Re-deriving
...
**Discharges:** [F75](../deferred-findings.md), and Plan.md's *Not in Phase 2*
...
- A key's effective permissions in an organization are its stored scopes
  ...
  *(Amended 2026-08-08 at [step 1](../phase-loop.md#1-validate)'s successor,
  [3.4](../phase-loop.md#3-land). This named `LoadMembershipAuthority` as the
...
- **[F103](../deferred-findings.md)'s bound** — a key's reads were bounded to
  ...
  *(Amended 2026-08-11 at [M57.5](m57.5.md), recording what [M58](m58.md)
  changed. As written this said the bound no longer applies to an account-wide
  key at all. Since M58 such a key's reads are bounded by the organizations it
  has been **cut out of**: [F183](../deferred-findings.md#closed) found
  ...
  [F178](../deferred-findings.md#closed) put those bars on the owner's own key
  list, so the narrowing is visible to the person it binds.)*
- **M44's `organization_id` parameter on `ResolveWorkspaceForUser`** — it exists
...
[M57.9](m57.9.md) covers this milestone, and it is the single strongest reason
