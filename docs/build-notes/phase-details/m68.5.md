# M68.5 — An add-on reaches outward, and only where the operator pointed it

**Depends on:** [M61](m61.md) *(the ABI this adds a function to, and the
deprecation policy that classifies the addition)*, [M62](m62.md) *(it costs a
declared permission)*, [M64](m64.md) *(settings are how an operator names an
origin)*, [M68](m68.md) *(the page that renders the permission and holds the
settings)*.
**Discharges:** [F334](../deferred-findings.md#open) — an add-on cannot make an
outbound network request, found at [M69](m69.md)'s validation and the reason
...
- **The operator names the origins, in settings.** An origin the host will
  dial comes from a declared setting the operator fills in — M68's page, M64's
  ...
  than trusting a default. The failure is a status the guest branches on and a
  log line naming the setting, not a trap.
...
- **Which invocations may fetch at all is decided here, and the redirect path
  ...
  guest call and spends that call's budget. For a **route** handler there is no
  ...
  so this milestone's first job is to say what bounds a fetching route handler
  and to build it. For a **redirect-class** invocation the budget is M66's
...
- **The redirect SLO is re-verified if and only if the redirect path ends up in
  ...
  `docs/slo.md` k6 measurement re-runs on the built image in M66.5's
  three-column shape and the well-behaved column stays under the 20ms bar M66.5
  was held to. If egress is refused there — the default above — the milestone
  says so in `docs/slo.md`'s M68 section instead, because *this cannot touch the
...
- **Attribution is per add-on**: `linkctrl_addon_fetch_duration_seconds{addon}`
  ...
  slowly, and what was refused. M68's manager renders them beside the redirect
  figures; this milestone records them.
- **The ABI addition is classified under M61's own policy**, which
  [M68](m68.md) already had to repair once: a new function is additive, the
...
- **`docs/SECURITY.md` gains the paragraph before the capability ships**, and
  ...
  **this is the first door in this product through which a server-side request
  ...
  validator has refused since M30 and what
  [M68.6](m68.6.md) will reuse. The relationship to that validator is stated:
...
- **The demo does not get it** (the inherited demo rule, claimed rather than
  ...
  arrives with [M68.6](m68.6.md) if it seeds anything at all. Stated here so
...
- **The operator's mental model is the other half of the security.** They name
  ...
  not make that consequence plain, the setting reads like a URL field. M68's
