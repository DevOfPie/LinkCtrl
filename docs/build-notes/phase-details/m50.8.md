...
- **This reverses `ListQRCodes`' stated design.** `internal/store/query/campaigns.sql:167`
  ...
  *Enforced by:* an `internal/link` test asserting the returned order for a set
...
- **Two records cite the old order and are part of this milestone, not
  ...
  reader is looking at"* — which alphabetical breaks — and
  `internal/link/qr.go:247` says the service *"returns every code a link
  carries, default first"*. **The promotion rule itself does not change**: D183
  set it to the oldest and nothing here reverses that; what changes is the
  sentence explaining why it looks natural, which stops being true. Both are
  corrected in this milestone's diff. *(Found by the plan review as a hidden
  edge.)*
...
- Every write redirects through `qrReturn` (`internal/httpx/web_qr.go:103`) and
  ...
  *Enforced by:* a browser case scrolling to the size control, saving, and
  asserting the control is still in the viewport.
...
- **This reverses what `partials/link_qr.html:266-269` argues** — *"Absent rather than
  ...
  *Enforced by:* a template test rendering a one-code link and a two-code link,
  ...
  (`partials/link_qr.html:545`) — but its reason is a visible `<p>` beside it at
  `:546-548`, not a tooltip, so this milestone follows its *disabled with a
