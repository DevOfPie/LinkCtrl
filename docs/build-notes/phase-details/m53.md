...
- **No outbound connection.** `docs/SECURITY.md`'s egress row stays at whatever
  count [M55](m55.md) leaves it at; this milestone adds nothing to it, and a
  test in the scan that already guards the destination judge is the mechanism.
...
- The interposition is between `RecordSuccessfulLogin`
  (`internal/auth/service.go:412`) and the session mint (`:430`) — the password
  is verified, and no session token exists until the second factor is.
