# M53 — A second factor: TOTP, enrolment, and recovery codes

**Depends on:** [M51](m51.md), and the edge is **hard**. A second factor makes
...
first writer". [M52](m52.md)'s erasure sweep landed the same day and clears both
...
- **No outbound connection.** `docs/SECURITY.md`'s egress row stays at whatever
  count [M55](m55.md) leaves it at; this milestone adds nothing to it.
  ...
  **The mechanism is a new scan, not an existing one**, and saying so is the
  ...
  M53's code lands in `internal/auth` and `internal/httpx`, which that test
...
- **The reason is stated because reuse is the tempting answer:** the pepper is
  ...
  [instances.md](../../dev-notes/instances.md) already documents that rotating
...
- The interposition is between `RecordSuccessfulLogin`
  ...
  M52 extracted `VerifyPassword` out of this file's password path and moved
...
### How M51 and M53 interact, decided here rather than discovered
...
happy-path one, and [M57.9](m57.9.md) is where that gets a second reader.
