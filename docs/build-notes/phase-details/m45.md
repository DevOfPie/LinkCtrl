...
**Depends on:** every milestone, and [M44.9](m44.9.md) having run to completion.
...
- **F1 is fixed and verified before the tag is pushed, not by pushing it.**
  ...
  `## [<version>]` to the next `## [` heading; the newest section is always last
  in CHANGELOG.md, so the trailing link-reference definitions fall inside that
  range and land in the published body. It is cosmetic — Markdown renders link
  *definitions* as nothing — and it will recur on every release, because the
  newest section is last every time.
