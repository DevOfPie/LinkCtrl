...
| **503** | Sixteen add-on requests are already in flight across the instance and this one waited out the request timeout. Each in-flight request holds a module instance, so this is capacity rather than an error | `linkctrl_http_requests_total` for the shape of the traffic; if it is not a burst, the add-on is slow and its own log will say why |
