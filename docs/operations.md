...
| `linkctrl_rate_limit_tracked_keys{limit}` | Client keys each limiter is holding **in this replica's own table**. For `login` and `api` that table is only written when the shared limiter did not answer, so a healthy shared limit reads **zero** here — which is indistinguishable from no traffic, and is why the next row exists. |
| `linkctrl_rate_limit_fallback_total{limit}` | Decisions made from this replica's own buckets because the shared limiter did not answer. **This is the series that says a shared limit has stopped being shared**: any movement means the configured number is being enforced per replica rather than across them. Always zero for the 404-probe limiter, which is deliberately not shared. |
...
| Credential limit stopped being shared | `rate(linkctrl_rate_limit_fallback_total{limit=~"login\|api"}[5m]) > 0` | The shared limiter is not answering, so each replica is enforcing the configured number on its own and N replicas allow N times it. A rate rather than a threshold on the value: the counter is monotonic, so `> 0` on the raw number latches forever after one transient blip. This row previously told you to watch a rate that "stays unchanged", which is a description rather than an expression ([F102](build-notes/deferred-findings.md)). |
