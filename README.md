...
- **A logo is composited, not verified.** Adding one forces error correction to
  level H so the code still scans under the occlusion, and the cap — a centred
  square one fifth of the symbol's width — is arithmetic rather than a
  measurement of your particular image. **Nothing checks that the result
  scans.** Forcing the level also grows the symbol, so the stored size is re-fitted
  to keep the PNG inside its ceiling, which means a code you sized at one number
  may download at another once it has a logo.
