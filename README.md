...
also publish static binaries for linux, macOS and Windows if you would rather
not use Docker; see [docs/releasing.md](docs/releasing.md).
...
- **A QR code does not follow your theme, and the PNG has a ceiling.** The
  picture paints its own white background across its quiet zone rather than
  going transparent: a QR code inverted onto a dark page is refused by a large
  share of scanners, so the drawing stays black on white in both themes and the
  frame around it is what the theme colours. You can restyle it — colours, error
  correction, size — and the form will let you choose a low-contrast pair if
  that is what your brand wants; it refuses only the two that are certainly
  broken, the same colour twice and anything that is not a hex colour. **The PNG
  stops at 2000px a side** and a request past it is refused rather than served,
  because the rasteriser allocates the whole image; the SVG has no such bound and
  prints at any size, which is the one to use for print. **Modules are square**:
  there is no rounded, dotted or custom module shape, and nothing schedules one.
- **A logo is composited, and the cap behind it is measured — but not your
  logo.** Adding one forces error correction to level H so the code still scans
  under the occlusion, and the cap — a centred square three tenths of the
  symbol's width — was chosen by decoding a corpus of every size and shape this
  product draws, at simulated distance, through two independent decoders. That
  is a measurement of the *cap*. **Nothing checks that your particular result
  scans**, and no such check runs at upload time or on a request. Forcing the
  level also grows the symbol, so the stored size is re-fitted to keep the PNG
  inside its ceiling, which means a code you sized at one number may download at
  another once it has a logo.
...
- **Deletion is a request with a lag, and erasure keeps the row.** An account is
  deleted immediately and *erased* by an hourly pass, so there is a window in
  which the account is gone from the product and its details are still in the
  database. Erasure scrubs the identifying fields and **keeps the row** rather
  than removing it, so foreign keys and audit records go on pointing at
  something — the audit trail of what an account did survives it, under
...
- **Account recovery needs a mailer, and there is no recovery for the instance
  principal's password.** With no `SMTP_HOST` a reset request refuses out loud
  and writes nothing — it does not queue, and there is nothing to switch on but
  a relay. The way back on a mailer-free instance is still you, with database
  access. `lctl instance principal move` hands the instance-administering role
  to another account from a shell, which repairs who administers the box rather
  than the password that was lost.
- **A second factor is optional, per account, and cannot be required.** It is
  unavailable at all until the operator sets `LINKCTRL_MFA_SECRET_KEY`, and
  losing that key means every enrolled account falls back to its recovery codes.
  There is no policy that obliges an organization's members to enrol, no
  WebAuthn, no passkeys, no SMS and no push. Recovery codes are ten, single-use,
  shown once.
