...
| **Domains** | A workspace registers a hostname of its own and administers it: register, rename, remove, from a page and from the API, behind `domains.write`. **A hostname belongs to exactly one workspace** — it is one alias namespace, so it cannot be shared and a second registration of the same name is refused instance-wide. An admin manages their own workspace's hostnames and gets `403` on anybody else's; the instance's default domain stays the operator's. **Nothing is served on a registered hostname yet**: it is stored unverified, no `Host` header is resolved against it, and an unrecognized host still gets the operational `404`. Proving control of a name and serving links on it are the next milestone. Registering, renaming and removing are audited. |
...
- **"Returning visitor" means earlier today and nothing longer.** The day ends
  ...
  keeps neither.
- **A registered domain serves nothing.** A workspace can register a hostname
  and own it, and that is the whole of it: the row is stored unverified, nothing
  checks that you control the name, no router resolves a `Host` header against
  it, and links keep using the instance's default domain. A hostname pointed at
  this instance gets the same `404` it got before you registered it. Verifying a
  domain and serving links on it are the next milestone.
