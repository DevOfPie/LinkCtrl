...
routing writes to Plan.md and to LNK findings and milestones, and a
...
| Already scheduled | A Plan.md row or LNK milestone covers it. Close the row pointing at the number. |
...
| --- | --- | --- |
| `issue` | An LNK finding (`mustur add finding --project LNK`) | One record: what, where as `file:line`, evidence it is real, suspected severity, **Found in** = the milestone the queue row carries. Evidence means verified against the tree — an unverified note becoming a findings row is exactly the laundering this queue exists to prevent. Unverifiable → prompt, do not weaken the column. |
...
| `task` | Its own commit, or [workflow-changes.md](../../docs/build-notes/workflow-changes.md) | Not a milestone, so per the scope gate it commits alone, when complete. **Made now** → commit, plus a *Made* row so it is findable without grepping git log. **Not made now** → a *Proposed* row, because a process change waiting must be as visible as a defect waiting. Anything non-obvious gets an LNK decision with **no** milestone number, naming what prompted it. |
...
- **A row leaves the queue only into a tracked file.** Every removal names its
  destination — an LNK finding, a Plan.md row and LNK milestone, a
  workflow-changes row, or an LNK decision recording that it was dropped and
...
- Anything decided while routing is an LNK decision, or the question is an LNK
  question. Routing forces judgement calls — which type a row is,
