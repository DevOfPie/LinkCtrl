...
| A registered domain serves nothing | [M39](docs/build-notes/phase-details/m39.md) is ownership and management only: a workspace registers a hostname and owns it, and the row is stored with `verified_at` NULL. Nothing checks that the registrant controls the name, no router resolves a `Host` header against the table, and a hostname pointed at the instance gets the same operational `404` it got before it was registered — asserted by `test/integration/hosts_test.go`, unchanged. Links stay on the instance's default domain. Verification and serving are [M40](docs/build-notes/phase-details/m40.md), and the split is what keeps the alias-hijack half reviewable on its own. |
