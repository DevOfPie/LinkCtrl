...
| Only a refusal with a blocklist row behind it is bounded to one open dispute | [M45](docs/build-notes/phase-details/m45.md) keys the "already waiting for review" bound on the entry the refusal matched, so every subdomain of one listed host is one dispute and one notification instead of one each. A refusal *computed from the URL* has no entry to count, so it is bounded by the host as typed — and `low_confidence.url_credentials` fires on credentials before the host with the host itself ignored, which means distinct strings are distinct disputes and each one notifies every organization owner on the instance over a browser route with no rate limiter. Narrowing the rule would change what the destination validator refuses, and the multiplier is the owner fan-out rather than the filer, so both repairs are decisions rather than corrections. M45 narrowed *who reviews* to the instance principal and its appointees (D98) and left `notify.EveryOwner` untouched, so the notification fan-out is still every organization owner on the instance. Recorded as finding F137. |
| A destination stored by an earlier build keeps the spelling it was stored with | The UTS-46 fold ([D93](#phase-2-decisions-taken-after-the-plan-was-finalised)) runs when a destination is **written**, and already-accepted links are never re-checked — which is *Not in Phase 2* above and predates this. So a link created before 0.2.0 whose host was written outside ASCII still holds and still serves that spelling, including one that would now be refused; editing the link converts it. There is no sweep, and building one is the separate job that row names. |
...
| Blocking is per link and per domain, and nothing between | The domain's setting is instance-wide, like the root redirect and the low-confidence blocklist, so an owner who enforces it decides for every workspace on the box — the shape `domains.write` already has, recorded as finding F70 and untouched by M45's principal, which took the dispute queue and the instance audit surface and nothing else. There is no per-workspace level, and no way to see how many refusals a setting is producing beyond `linkctrl_redirects_total{outcome="blocked_bot"}` and the bot column of a link's own statistics. |
