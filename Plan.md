...
- **Account deletion and erasure, of any kind, for anybody.** There is no way to
  ...
  **Discharged by [M52](docs/build-notes/phase-details/m52.md) on 2026-08-08**,
  ...
  ([D148](#phase-3-decisions)). **The dispute pair has one open exception at
  0.3.0**, [F187](docs/build-notes/deferred-findings.md#open): the two columns are
  written by two statements, and a row filed *and* decided by accounts erased in
  the same batch keeps one of its labels because Postgres applies one of the two
  and drops the other. It is a defect of M52 rather than of the sweep's later
  widening, which is why [D175](#phase-3-decisions) reopens M52 for it instead of
  carrying it. **One limb of the row survives on purpose**:
