...
- **Account deletion and erasure, of any kind, for anybody.** There is no way to
  ...
  **Discharged by [M52](docs/build-notes/phase-details/m52.md) on 2026-08-08**,
  ...
  `audit_logs` and **both** label columns in `destination_disputes`, then stamps
  `anonymized_at`. The tombstone is a constant and the ids survive
  ([D148](#phase-3-decisions)). **Two limbs of the row survive on purpose.** The
  first is that `click_events` and `visitors` are still outside erasure and still
  need to be — there is no account identity in them to erase. The second is
  narrower and newer: erasure reaches the actor snapshot and not an address
  carried in an audit record's `metadata` as the *subject* of somebody else's
  action, which is [F177](docs/build-notes/deferred-findings.md) and is stated in
  `docs/SECURITY.md` rather than left for a reader to find.
